...
每个至少已有一次复核通过（可以是当时的源版本，否则返回 409 并列出从未通过的事项）；evidenceIds \
必须包含至少一项绑定当前源版本的 execution 证据（先 execution.prepare，再在同一原生轮次跑完整检查），否则返回 \
400。复核者会核对这些检查确实属于当前源码，并逐个事项检查其复核后的改动有没有推翻原结论。\
