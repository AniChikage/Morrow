 ...
 */
/** One feature or action under review; every field is already serialized. `minutes` is the row's own cap. */
export type ItemReviewFields = {
  subject: string;
  progress: string;
  version: string;
  evidence: string;
...
在最多 ${p.minutes} 分钟内选择能推翻当前结论的检查。独立读取实现与真实存储/输入格式，检查边界、分母/分子口径、\
