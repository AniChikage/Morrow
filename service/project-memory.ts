...
export class ProjectMemory {
  ...
  describe(kind: MemoryKind, row: MemoryRecord): MemoryMatch {
    ...
    const excerpt =
      understanding?.statement || decision?.review?.conclusion || learning?.conclusion || learning?.rationale || '';
    const caution = understanding
      ? understanding.status !== 'active'
        ? '已失效或停用，只能作为历史参考'
        : understanding.reviewAt <= now()
          ? '已到复查时间，不能直接沿用'
          : understanding.kind === 'assumption' || understanding.kind === 'unknown'
            ? '尚未确认的认识，需要验证'
            : undefined
      : decision
        ? '这是当时条件下的复盘，需重新判断适用性'
        : learning?.status === 'refuted' || learning?.status === 'stopped'
          ? '已被推翻或停止，不能直接沿用'
          : learning?.status !== 'supported'
            ? '尚无确定结论，需重新验证'
            : '证据支持限于当时条件';
    return {
      kind,
  ...
  recall(scope: Scope, input: Record<string, unknown>) {
    ...
    const query = string(
      input.query ??
        [item?.title, active?.options[active.selected].title, channel.work?.focus, project.goal, channel.goal]
          .filter(Boolean)
          .join('\n')
          .slice(0, 4000),
      'query',
    ...
    const corpus = (Object.keys(memoryTables) as MemoryKind[]).flatMap((kind) =>
      this.loop
        .rows<MemoryRecord>(memoryTables[kind], project.id)
        ...
        .map((row) => {
          ...
          const body =
            kind === 'understanding'
              ? [
                  (row as Understanding).statement,
                  (row as Understanding).relevance,
                  (row as Understanding).verification,
                ]
              : kind === 'decision'
                ? [
                    (row as StrategyDecision).rationale,
                    (row as StrategyDecision).nextStep,
                    (row as StrategyDecision).expectedOutcome,
                    (row as StrategyDecision).review?.conclusion,
                    (row as StrategyDecision).review?.nextDirection,
                    ...((row as StrategyDecision).expectations?.flatMap((e) => [e.claim, e.scope, e.disconfirm]) || []),
                    (row as StrategyDecision).review?.assessment?.conditionReason,
                    (row as StrategyDecision).review?.assessment?.explanation,
                  ]
                : [
                    (row as Learning).rationale,
                    (row as Learning).expectedResult,
                    (row as Learning).evaluation,
                    (row as Learning).conclusion,
                  ];
          return { row, match, title: terms(match.title), body: terms(body.filter(Boolean).join('\n')) };
        })
    );
    const frequencies = new Map(
      queryTerms.map((term) => [term, corpus.filter((doc) => doc.title.has(term) || doc.body.has(term)).length])
    );
    const ranked = corpus
      .map((doc) => {
        ...
        const score =
          matched.reduce(
            (total, term) =>
              total + Math.log(1 + corpus.length / (1 + (frequencies.get(term) || 0))) * (doc.title.has(term) ? 3 : 1),
            0
          ) + (sameFeature ? 12 : 0);
        const reasons = [
          ...(sameFeature ? ['同一 feature 的历史记录'] : []),
          ...(matched.length ? [`文字相关：${matched.slice(0, 5).join('、')}`] : []),
        ];
      ...
      .sort(
        (a, b) =>
          b.score - a.score ||
          b.match.updatedAt.localeCompare(a.match.updatedAt) ||
          a.match.id.localeCompare(b.match.id)
      );
    return {
      query,
      matches: ranked.slice(0, limit).map((row) => row.match),
      hasMore: ranked.length > limit,
      method:
        '项目内文字相关性与同一 feature 关联；排序不代表事实可信度或经验适用性。先用 memory.read 阅读完整记录，再判断是否引用。',
    };
  }
  references(scope: Scope, value: unknown): MemoryReference[] {
    ...
    return value.map((raw) => {
      ...
      keys(input, ['kind', 'id', 'revision', 'use', 'reason']);
      const kind = choice(input.kind, 'kind', Object.keys(memoryTables) as MemoryKind[]);
      const id = string(input.id, 'id', 200),
        row = this.record(scope.projectId, kind, id);
      ...
      if (
        use === 'apply' &&
        (['invalidated', 'retired', 'refuted', 'stopped'].includes(row.status) ||
          (kind === 'understanding' && (row as Understanding).reviewAt <= now()) ||
          ['not_improved', 'abandoned'].includes(snapshot.outcome || ''))
      ) {
