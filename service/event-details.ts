...
export function sanitizeEventDetail(
  value: EventDetail | undefined,
  ...
  const visit = (value: unknown, depth: number): unknown => {
    ...
    if (value && typeof value === 'object') {
      ...
      for (const [key, item] of Object.entries(value).slice(0, 40)) {
        ...
        result[safeKey] =
          /(?:authorization|api[_-]?key|^token$|access[_-]?token|refresh[_-]?token|password|secret)/i.test(safeKey)
            ? '[REDACTED]'
            : visit(item, depth + 1);
      }
      return result;
...
export function providerEventDetails(data: any): EventDetail[] {
  ...
  if (item && typeof item === 'object') {
    const status =
      typeof item.status === 'string' ? item.status : data.type === 'item.completed' ? 'completed' : 'running';
    ...
    if (item.type === 'mcp_tool_call')
      return [
        {
          type,
          tool: [item.server, item.tool].filter((v) => typeof v === 'string').join('.'),
    ...
    if (item.type === 'file_change')
      return [{ type, tool: 'apply_patch', input: { changes: item.changes }, status, toolCallId }];
    if (item.type === 'web_search')
      return [{ type, tool: 'web_search', input: { query: item.query, action: item.action }, status, toolCallId }];
    if (item.type === 'tool_call' || item.type === 'tool_use' || item.type === 'tool_result')
      return [
        {
          type: item.type === 'tool_result' ? 'tool_result' : type,
  ...
  return blocks.slice(0, 40).map((block: any): EventDetail =>
    block.type === 'tool_use'
