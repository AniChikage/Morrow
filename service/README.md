...
自动轮次获得项目目标、方向、共享看板、已有认识、相关经验和人工指导，以及 `service/native-capabilities.ts` 里那份 2026-09-09 实测的原生能力清单（`contract.nativeCapabilities` 和任务章程里的一行）。同一频道沿用同一原生任务，因此提示词分成两部分：任务开头一次性发出的**章程**（角色、目标、项目说明全文及版本、工作方向、规则与授权范围），以及之后每轮只发的**轮次提示**（一行提醒、上次安排、额度、看板摘要和本轮工具入口）。章程在项目说明/目标/方向/权限变化、换绑原生任务、上一轮未正常结束或没有保存 morrow-next、以及同一章程用满 10 轮时重发；被原生任务拒收的轮次不算已送达。看板以 `#编号 类型 状态 标题` 摘要给出（人建立的事项和上一轮改动过的事项附下一步，已解决只计数），完整字段用 `context` 读取。Codex 通过运行范围内的工作接口维护项目；下一步可选择继续、等待或提问。用户手动暂停优先于反馈唤醒。
