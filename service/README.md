...
所有表位于 `workspace.sqlite`。`runs/`、`native-images/` 和 `releases/` 保存相关私有文件；`runtime/<整包指纹>/` 保存本次运行的 daemon 自己那两份辅助脚本副本——工作接口 `agent-cli.ts` 与复核工作进程 `codex-cli-worker.ts`（见[升级与数据迁移](../docs/UPGRADING.md)的混版窗口一节），启动成功后其他指纹的副本会被删掉。原生任务的权威历史由 Codex 管理，Morrow 的 SQLite 保存已同步的镜像和编排记录，不把自己的记录当成另一套原生会话。
...
- 上下文在轮次边界自动压缩：任务上下文用量（同步状态里的 `last.totalTokens / modelContextWindow`）达到 `compactContextPercent`（65%）时，轮次开始前先请求 App 压缩并等待其完成（最长 `compactWaitMs`，默认 180 秒；仍在压缩则调度置 `waiting` 按 5 秒重试，手动运行返回 409），轮次结束后再补一次不等待的请求，压缩完成即自动继续工作，不需要人来停轮次再 /compact。读数缺失或不可用时不压缩；压缩被拒或未在等待时间内完成只写一条系统事件，本轮照常开始，App 自己的自动压缩仍是兜底。压缩只发生在轮次边界，从不为此打断轮次——中途超过阈值的轮次照常跑完。App 自己或人工完成的压缩按条目 ID 去重，各写一条系统事件。完成记录只写一次，且只写在同步到的那条已完成压缩条目上：Morrow 在 10 分钟内请求过的算按 65% 规则压缩（连 `native.compacted` 审计一并写在这里），否则算 App 自己或人工压缩——App 会把跟随端的请求记成人工 /compact，所以不能由请求方自己记。
- 只读/工作区选项保留 `on-request` + `auto_review` 和明确沙箱；发送 `permissions:null` 以配合显式沙箱。App 可能合并已有目录，启动前仍检查当前权限，不能声称只含传入目录。
- 独立复核走官方 `codex exec` 一次性临时会话，使用专用 supervisor 处理取消、硬超时及父服务退出。只有实际 CLI 工具事件、正常终止和退出码均有效时才能通过。CLI 未提供轮次 ID 时保持为空，不编造原生 ID。沙箱由 `reviewArguments(model, isolated)` 决定：没有隔离检出时仍是 `--sandbox read-only`（命令行与以前逐字一致）；在隔离检出里跑时换成 `--sandbox workspace-write`，写入范围就是那个用完即删的检出，并显式加 `-c sandbox_workspace_write.network_access=false`——`read-only` 隐含不联网，换掉之后必须自己关。其余不变：`approval_policy="never"`、`--ephemeral`、`--ignore-user-config`、关闭 Web 搜索与 MCP。注意 `fileChange` 观察仍然一律判「超出只读范围」：检出里的写入请用命令完成，模型若用补丁类工具改文件，这次复核照旧停下判未知。
- 复核不与执行者同一运行时：`WorkVerification.reviewRunner` 在两个已接入的运行器里挑不是本频道运行时的那个（Codex/Trae 频道 → Claude Code 复核，Claude Code 频道 → Codex 复核），目标运行时没装在本机就回退到 Codex 复核，实际用的那个记在行上的 `executionOwner`（`codex-cli` / `claude-cli`）。Claude Code 复核是 `service/claude-cli-review.ts` 的一次有界 `claude --print` 轮次（stream-json、`--safe-mode`、空 MCP、`dontAsk`、`--name Morrow:review-<id>`，工具表 `Read,Grep,Glob`，只有在隔离检出里才加 `Bash`），与 Codex 复核共用同一个 supervisor；它不花 Codex 额度，因此 `usage.gate` 不对它生效，它自己的额度用尽也只让这一行等待重试，不写 Codex 的 `exhaustedUntil`。
- Claude Code 跑不起来时**在同一次 `start()` 里回退到 Codex 复核**，不判 unknown：安装检测不能证明登录仍然有效，而复核是事项唯一的认证通道，一次掉登录会让所有事项停在调查中，且看起来像复核给出的结论。触发条件只有三种「运行时不可用」：子进程起不来（ENOENT/EACCES/spawn 失败，含运行器自己抛出的「未找到 Claude Code 命令行」）、`diagnoseFailure` 判为登录失效（优先级 100）、以及 stdout 不是合法 JSONL 且一条 item 都没解析出来。回退只发生一次，且只有 Claude → Codex 这一个方向；真正跑过的复核（包括额度用尽和「CLI 未正常完成」）仍然是它自己的结果。回退时这次失败的观察被丢弃（`seen` 清空、`bytes`/`commandCount` 归零），检出复用不重建，`executionOwner` 改记成真正执行的 `codex-cli`，并写一条 `verification.runtime-unavailable` 系统事件说明这是本机环境问题；因为 Codex 要花额度，回退前重新过一次 `usage.gate`，被挡住就把行退回 `queued` 并按 `holdForUsage` 等额度（释放检出、清掉定时器与 `active`），而不是当成失败。
- 复核尽量在**一次性隔离检出**里跑（`service/review-checkout.ts`）：启动前用 `git worktree add --detach` 把被复核的那个源版本检出到 `MORROW_HOME/reviews/<复核 ID>`，把项目的 `node_modules` 软链进去，复核进程的 cwd 指向它，复核到达任一终局（通过、反例、未知、取消、超时、异常）即 `git worktree remove --force` 并删除目录；服务重启时 `WorkVerification.recover()` 清掉被强杀留下的目录与注册。只有项目是 Git 仓库、工作树干净（未提交或未跟踪的文件都会让检出与提交不一致）、当前源版本读数仍等于该行封存的 digest 且提交可解析时才建检出，否则**回退**到在项目目录里跑，行为与之前完全一致。「版本时效」规则（`current(row)`）不受影响，检出只改变复核者在哪看、能跑什么。用了检出时，**两个 CLI 运行器**的提示词末尾追加 `isolatedReviewText`（告诉复核者它在该版本的一次性检出里、可以自己重跑格式检查/类型检查/测试/构建并以亲自运行的结果为依据），并把 `isolated` 传给运行器：Codex 换成可写工作区沙箱，Claude Code 的工具表追加 `Bash`。**原生 transport 那条复核路径不追加这段话**，也继续发 `sandboxPolicy: {type:'readOnly', networkAccess:false}`：App 会把传入的 `writableRoots` 与它自己保留的目录合并，Morrow 无法保证写入只落在检出里，提示词不能承诺运行时做不到的事；它仍然在检出里读被复核的那个版本。回退路径（没有检出）仍是原来「不能写文件、联网、安装依赖」的措辞和只读沙箱。
...
Claude 工作轮次使用 `--print --verbose --output-format stream-json --setting-sources user --strict-mcp-config --mcp-config <Morrow-only> --tools <T> --allowedTools <T>,mcp__morrow__call --permission-mode <M> --name Morrow:<runId>`，有模型时加 `--model`，有会话时加 `--resume <sessionId>`。只读为 `T = Read,Grep,Glob`、`M = dontAsk`；工作区写入为 `T = Read,Grep,Glob,Edit,Write,MultiEdit,NotebookEdit,Bash`、`M = acceptEdits`。不使用 `--safe-mode`：实测它会丢掉 `--mcp-config`。`--setting-sources user` 跳过项目/local 的 CLAUDE.md 与 hooks，但弱于 `--safe-mode`（user 级插件/技能/hooks 仍可能加载）。复核仍用 `--safe-mode` 和空 MCP。会话 ID 取事件流里的 `session_id`。
