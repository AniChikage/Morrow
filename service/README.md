...
远程项目使用远端绝对路径，数据与执行保留在远端；Codex 直接使用远端 CLI 与登录配置，不回连 Mac App。具体远端主机仍需单独验收。
...
同一目录只允许一个 daemon，通过 `daemon.lock` 防止重复实例。SIGTERM/SIGINT 会停止服务调度并清理其拥有的 CLI 进程；不操作 Codex App。崩溃后，未结束的自有 CLI 轮次标记中断并暂停频道；历史 App 任务保留原始记录。发送回执不明确时先核对结果，不盲目重发。

## 原生运行时

### Codex CLI

Morrow 直接启动 `codex app-server --listen stdio://`，通过逐行 JSON-RPC 初始化、创建/恢复任务和订阅执行事件。该进程属于 Morrow，不发现或联动 Codex App 后台，不依赖桌面应用生命周期。使用执行主机现有 CLI 认证、模型和工具配置；不复制 App 专属配置。

- 每个频道绑定同一项目目录下的原生任务，按原 ID 恢复；不丢失旧 SQLite 关联与历史。
- 普通对话传递原始文字、图片与请求 ID，执行中通过原生 steering 追加指导。
- 自动执行显式应用频道沙箱范围与原生自动审查；原生权限模式继承 CLI 配置。独立复核也使用同一个 CLI 接入层，创建独立只读任务。
- 审批与问答按原生请求 ID 回答；不支持的协议请求显示详情和错误，不引导启动 App。
- PNG/JPEG/WebP/GIF 单张最多 10 MiB，每批最多 5 张、20 MiB；附件保留频道隔离与摘要校验。
- 断线保留历史及未知回执，先核对结果，不自动重复发送；恢复不会偷偷更换原生任务 ID。
- 退出只结束 Morrow 自己启动的 CLI 子进程，不停止用户的终端 CLI 或 Codex App。

旧 App 桥接代码仅保留历史兼容测试与撤销功能，生产默认入口不再加载它。桥接设置接口已停用；撤销自有启动项不会终止正在运行的 App，也不需要重启 App。
...
同一项目目录的责任轮次串行执行，并等待已绑定原生任务空闲。每日上限按 UTC 日界计算，已启动的失败/中断轮次也计数；普通对话和历史外部轮次不消耗编排预算。配置范围为每天 1–100 轮、复查间隔 1–1440 分钟。

自有 CLI 单轮超时 15 分钟，stdout/stderr 合计上限 20 MiB，组装提示上限 1 MiB；这些子进程限制不直接套用到Codex JSON-RPC 轮次。暂停原生自动工作只中断属于该责任轮次的精确 turn ID。
