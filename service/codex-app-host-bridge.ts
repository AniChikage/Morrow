...
export function sharedRuntimeArgs(args: string[], socket: string): string[] | null {
  ...
  while (commandIndex < args.length && args[commandIndex] !== 'app-server') {
    ...
    if (['-c', '--config', '--enable', '--disable'].includes(arg)) {
      ...
      commandIndex += 2;
    } else if (/^(?:--config|--enable|--disable)=.+/.test(arg) || /^-c.+?=.+/.test(arg) || arg === '--strict-config')
      commandIndex++;
    else return null;
  }
  if (
    args[commandIndex] !== 'app-server' ||
    args
      .slice(commandIndex + 1)
      .some((arg) => ['daemon', 'proxy', 'generate-ts', 'generate-json-schema', '--help', '-h'].includes(arg))
  )
    return null;
  ...
  for (let index = commandIndex + 1; index < args.length; index++) {
    ...
    if (arg === '--listen') {
      if (args[++index] !== 'stdio://') return null;
      continue;
    }
    if (arg.startsWith('--listen=')) {
      if (arg !== '--listen=stdio://') return null;
      continue;
    }
    result.push(arg);
  }
...
export async function runAppHostBridge(args = process.argv.slice(2)): Promise<void> {
  ...
  if (!executable || resolve(executable) === resolve(process.argv[1]))
    throw new Error('Morrow Codex bridge has no native executable.');
  const directory =
    process.env.MORROW_CODEX_BRIDGE_HOME ||
    process.env.NOHUMAN_CODEX_BRIDGE_HOME ||
    join(homedir(), 'Library/Application Support/Morrow/codex-bridge');
  const launchId = randomUUID();
  // Unix socket paths have a small OS limit. The private directory is kept short.
  const socketDirectory =
    process.env.MORROW_CODEX_SOCKET_DIRECTORY ||
    process.env.NOHUMAN_CODEX_SOCKET_DIRECTORY ||
    join(homedir(), '.morrow-codex');
  ...
  const cleanup = () => {
    ...
    try {
      ...
      /* This process only removes its own receipt. */
    }
  };
  ...
  child.once('error', (error) => {
    process.stderr.write(`Morrow native host failed: ${error.message}\n`);
    process.exitCode = 1;
    cleanup();
  });
  child.once('exit', (code) => {
    process.exitCode = code ?? (stopped ? 0 : 1);
    cleanup();
  });
  try {
    for (let count = 0; count < 300 && !stopped && !existsSync(socketPath); count++)
    ...
    socket.on('message', (data) => {
      ...
      if (!initialized) {
        try {
          ...
          /* Runtime JSON is forwarded intact; the App handles protocol errors. */
        }
      }
    ...
    socket.once('error', (error) => {
      process.stderr.write(`Morrow native connection failed: ${error.message}\n`);
      process.exitCode = 1;
      cleanup();
    });
