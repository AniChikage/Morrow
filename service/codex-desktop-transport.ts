...
export type NativeThreadChange =
  | { type: 'snapshot'; revision: number; conversationState: Record<string, any> }
  | {
      type: 'patches';
      baseRevision: number;
      revision: number;
      patches: Array<{ op: 'add' | 'replace' | 'remove'; path: Array<string | number>; value?: unknown }>;
    };
...
interface Subscription {
  owner: string | null;
  snapshot: NativeThreadSnapshot | null;
  error: NativeDesktopError | null;
  listeners: Set<(snapshot: NativeThreadSnapshot) => void>;
  changeListeners: Set<(snapshot: NativeThreadSnapshot, change: NativeThreadChange) => void>;
  waiting: Set<{
    resolve: (snapshot: NativeThreadSnapshot) => void;
    reject: (error: Error) => void;
    copy: boolean;
    timer: ReturnType<typeof setTimeout>;
  }>;
...
export class NativeDesktopError extends Error {
  ...
  constructor(message: string, code = 'desktop_unavailable', outcomeUnknown = false) {
    super(message);
    this.name = 'NativeDesktopError';
    this.code = code;
...
export function applyDesktopPatches(state: Record<string, any>, patches: unknown): Record<string, any> {
  ...
  const copy = (value: any): any => {
    if (value == null || typeof value !== 'object')
      throw new NativeDesktopError('无效的原生对话增量路径。', 'protocol_mismatch');
  ...
  for (const patch of patches) {
    if (
      !patch ||
      !['add', 'replace', 'remove'].includes(patch.op) ||
      ...
      throw new NativeDesktopError('原生对话增量格式不受支持。', 'protocol_mismatch');
    if (patch.path.length === 0) {
      if (patch.op === 'remove' || !patch.value || typeof patch.value !== 'object')
        throw new NativeDesktopError('无效的原生对话快照。', 'protocol_mismatch');
    ...
    for (const key of patch.path.slice(0, -1)) {
      if (parent == null || typeof parent !== 'object' || !Object.hasOwn(parent, key))
        throw new NativeDesktopError('原生对话增量缺少前置版本。', 'revision_gap');
    ...
    if (Array.isArray(parent) && key !== 'length') {
      ...
      if (
        ...
        index < 0 ||
        index > parent.length ||
        (patch.op !== 'add' && index >= parent.length)
      )
        throw new NativeDesktopError('无效的原生对话数组增量。', 'protocol_mismatch');
...
function nativeInput(text: string, images: NativeImageInput[] = []): Array<Record<string, unknown>> {
  if (
    ...
    images.some((image) => typeof image?.path !== 'string' || !isAbsolute(image.path) || image.path.includes('\0'))
  )
    throw new NativeDesktopError('原生图片必须使用有效的本地绝对路径。', 'invalid_image');
...
export class CodexDesktopTransport {
  ...
  constructor(options: Options = {}) {
    ...
    this.socketPath = options.socketPath ?? join(this.codexHome, 'ipc', 'ipc.sock');
    this.timeout = options.requestTimeoutMs ?? 15_000;
  }
  status(): NativeDesktopStatus {
    return {
      connected: !!this.socket && !this.socket.destroyed && this.clientId !== 'initializing-client',
  ...
  threadStatus(threadId: string): { ready: boolean; detail: string; lastSyncedAt?: string } {
    ...
    return {
      ready: this.status().connected && !!sub?.owner && !!sub.snapshot && !sub.error,
      detail: sub?.error?.message ?? (sub?.snapshot ? '已同步原生任务。' : '尚未收到当前原生任务快照。'),
      ...(sub?.snapshot ? { lastSyncedAt: sub.snapshot.syncedAt } : {}),
    };
  }
  ...
  private async connectInternal(): Promise<void> {
    if (!this.validateSocket(this.socketPath) && !this.options.socketPath) {
      const legacy = join(tmpdir(), 'codex-ipc', `ipc-${process.getuid?.()}.sock`);
      if (this.validateSocket(legacy)) this.socketPath = legacy;
    }
    if (!this.validateSocket(this.socketPath)) {
      this.lastError = '未找到 Codex App 的本机对话连接，请打开 Codex App。';
      throw new NativeDesktopError(this.lastError);
    }
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        const timer = setTimeout(() => {
          socket.destroy();
          reject(new NativeDesktopError('连接 Codex App 超时。'));
        }, this.timeout);
        socket.once('connect', () => {
          clearTimeout(timer);
          resolve();
      ...
      if (typeof response.result?.clientId !== 'string')
        throw new NativeDesktopError('Codex App 初始化协议不兼容。', 'protocol_mismatch');
      this.clientId = response.result.clientId;
      this.lastError = null;
      for (const [threadId, sub] of this.subscriptions)
        if (sub.listeners.size + sub.changeListeners.size)
          void this.attach(threadId, sub).catch((error) => {
            this.lastError = error.message;
          });
    } catch (error) {
      socket.destroy();
  ...
  private write(message: any): void {
    ...
    const frame = encodeDesktopFrame(message);
    if (frame.length - 4 > (this.options.maxFrameBytes ?? 256 * 1024 * 1024))
      throw new NativeDesktopError('原生对话数据超过协议限制。', 'frame_too_large');
  ...
  private request(
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(
          new NativeDesktopError(
            mutation ? 'Codex App 未及时确认操作，结果未知；请等待同步，勿重复发送。' : 'Codex App 响应超时。',
            'request_timeout',
            mutation
          )
        );
  ...
  private clearIncoming(): void {
    this.incomingHeaderBytes = 0;
    this.incomingFrame = null;
    this.incomingFrameBytes = 0;
  }
    ...
    try {
      while (offset < chunk.length && !this.disposed && socket === this.socket) {
        if (!this.incomingFrame) {
          ...
          chunk.copy(this.incomingHeader, this.incomingHeaderBytes, offset, offset + count);
          this.incomingHeaderBytes += count;
          offset += count;
          if (this.incomingHeaderBytes < 4) return;
          const size = this.incomingHeader.readUInt32LE(0);
          this.incomingHeaderBytes = 0;
          if (!size || size > (this.options.maxFrameBytes ?? 256 * 1024 * 1024))
            throw new NativeDesktopError('Codex App 数据帧不受支持。', 'protocol_mismatch');
          this.incomingFrame = Buffer.allocUnsafe(size);
          this.incomingFrameBytes = 0;
        }
        const count = Math.min(this.incomingFrame.length - this.incomingFrameBytes, chunk.length - offset);
        chunk.copy(this.incomingFrame, this.incomingFrameBytes, offset, offset + count);
        this.incomingFrameBytes += count;
        offset += count;
  ...
  private handle(message: any): void {
    if (message.type === 'response') {
      ...
      if (message.resultType === 'error')
        pending.reject(
          new NativeDesktopError(
            message.error === 'no-client-found' ? '请先在 Codex App 中打开此对话，再连接同步。' : String(message.error),
            message.error
          )
        );
    ...
    if (message.method === 'client-status-changed' && message.params?.status === 'disconnected') {
      for (const [id, sub] of this.subscriptions)
        if (sub.owner === message.params.clientId) {
          sub.owner = null;
          sub.snapshot = null;
          sub.error = new NativeDesktopError('原生任务所有者已断开，正在重新连接。');
          if (sub.listeners.size + sub.changeListeners.size) this.scheduleReconnect(id);
        }
      return;
    }
    const params = message.params;
    if (params?.hostId !== 'local' || typeof params.conversationId !== 'string') return;
    const sub = this.subscriptions.get(params.conversationId);
    if (!sub) return;
    if (
      message.method === 'thread-stream-following-status-requested' &&
      message.version === 1 &&
      sub.listeners.size + sub.changeListeners.size &&
      sub.owner === message.sourceClientId
    ) {
      this.follow(params.conversationId, sub, true);
      return;
    }
    if (message.method !== 'thread-stream-state-changed' || message.sourceClientId !== sub.owner) return;
    if (message.version !== BROADCAST_VERSION) {
      this.lastError = 'Codex App 对话协议版本已变化，请更新 Morrow。';
      sub.snapshot = null;
      sub.error = new NativeDesktopError(this.lastError, 'protocol_mismatch');
      this.failWaiters(sub, sub.error);
      return;
    }
    ...
    try {
      ...
      if (change.type === 'snapshot') {
        if (!change.conversationState || change.conversationState.id !== params.conversationId)
          throw new Error('thread mismatch');
        state = change.conversationState;
      } else if (
        change.type === 'patches' &&
        sub.snapshot &&
        change.baseRevision === sub.snapshot.revision &&
        change.revision > change.baseRevision
      )
        state = applyDesktopPatches(sub.snapshot.state, change.patches);
      else {
        this.refreshSnapshot(params.conversationId, sub);
        return;
      }
      sub.error = null;
      sub.snapshot = {
        threadId: params.conversationId,
        ownerClientId: sub.owner!,
      ...
      for (const listener of sub.listeners) {
        try {
          listener(structuredClone(sub.snapshot));
        } catch {
          /* A consumer failure must not close the native transport. */
        }
      }
      for (const listener of sub.changeListeners) {
        try {
          listener(sub.snapshot, change);
        } catch {
          /* Consumer owns persistence/retry; never mutate this shared snapshot. */
        }
      }
      for (const waiter of sub.waiting) {
        clearTimeout(waiter.timer);
        waiter.resolve(waiter.copy ? structuredClone(sub.snapshot) : sub.snapshot);
      }
      sub.waiting.clear();
    } catch {
      this.lastError = '原生对话增量无法应用，正在重新同步。';
      this.refreshSnapshot(params.conversationId, sub);
    }
  }
  private disconnected(socket: Socket): void {
    ...
    this.clientId = 'initializing-client';
    this.clearIncoming();
    this.lastError ??= 'Codex App 已断开。';
    for (const pending of this.pending.values()) {
      clearTimeout(pending.timer);
      pending.reject(
        new NativeDesktopError(
          'Codex App 连接已断开。',
          pending.mutation ? 'delivery_unknown' : 'desktop_unavailable',
          pending.mutation
        )
      );
    }
    ...
    for (const sub of this.subscriptions.values()) {
      sub.owner = null;
      sub.snapshot = null;
      sub.error = new NativeDesktopError('Codex App 连接已断开。');
      this.failWaiters(sub, new NativeDesktopError('Codex App 连接已断开。'));
    }
    if (!this.disposed && [...this.subscriptions.values()].some((sub) => sub.listeners.size + sub.changeListeners.size))
  ...
  private scheduleReconnect(threadId?: string): void {
    ...
    this.reconnectTimer = setTimeout(() => {
      ...
      void this.connect()
        .then(async () => {
          for (const [id, sub] of this.subscriptions)
            if (sub.listeners.size + sub.changeListeners.size && !sub.owner) await this.attach(id, sub);
        })
        .catch((error) => {
          this.lastError = error.message;
          this.scheduleReconnect();
  ...
  private failWaiters(sub: Subscription, error: Error): void {
    for (const waiter of sub.waiting) {
      clearTimeout(waiter.timer);
      waiter.reject(error);
    }
  ...
  private follow(threadId: string, sub: Subscription, following: boolean): void {
    ...
    this.write({
      ...
      version: 1,
      sourceClientId: this.clientId,
      targetClientIds: [sub.owner],
      params: { conversationId: threadId, hostId: 'local', following },
    });
  }
  private refreshSnapshot(threadId: string, sub: Subscription): void {
    sub.snapshot = null;
    sub.error = new NativeDesktopError('正在重新同步原生任务。', 'resyncing');
    this.follow(threadId, sub, false);
    this.follow(threadId, sub, true);
  }
  private async attach(threadId: string, sub: Subscription): Promise<void> {
    ...
    sub.attaching = (async () => {
      ...
      if (typeof response.handledByClientId !== 'string') throw new NativeDesktopError('Codex App 对话所有者不可用。');
      sub.owner = response.handledByClientId;
      sub.error = null;
      this.follow(threadId, sub, true);
    })().finally(() => {
      sub.attaching = undefined;
    });
    return sub.attaching;
  }
  private subscription(threadId: string): Subscription {
    ...
    let sub = this.subscriptions.get(threadId);
    if (!sub) {
      sub = {
        owner: null,
        snapshot: null,
  ...
  async readThread(threadId: string): Promise<NativeThreadSnapshot> {
    return this.readThreadSnapshot(threadId, true);
  }
  private async readThreadSnapshot(threadId: string, copy: boolean): Promise<NativeThreadSnapshot> {
    await this.connect();
    const sub = this.subscription(threadId);
    ...
    return new Promise((resolve, reject) => {
      const waiter = {
        ...
        copy,
        timer: setTimeout(() => {
          sub.waiting.delete(waiter);
          reject(new NativeDesktopError('Codex App 对话快照响应超时。'));
        }, this.timeout),
      };
      sub.waiting.add(waiter);
    });
  }
  ...
  async subscribeChanges(
    threadId: string,
    listener: (snapshot: NativeThreadSnapshot, change: NativeThreadChange) => void
  ): Promise<() => void> {
    const sub = this.subscription(threadId);
    let delivered = false;
    const receive = (snapshot: NativeThreadSnapshot, change: NativeThreadChange) => {
      delivered = true;
    ...
    try {
      ...
      if (!delivered)
        receive(snapshot, { type: 'snapshot', revision: snapshot.revision, conversationState: snapshot.state });
    } catch (error) {
      sub.changeListeners.delete(receive);
      throw error;
    }
    return () => {
      sub.changeListeners.delete(receive);
  ...
  async loadCompleteHistory(threadId: string): Promise<NativeThreadSnapshot> {
    ...
    const sub = this.subscription(threadId);
    if (sub.snapshot && sub.snapshot.revision >= response.result?.revision) return structuredClone(sub.snapshot);
    this.refreshSnapshot(threadId, sub);
  ...
  async listThreads(cwd: string): Promise<NativeThreadSummary[]> {
    ...
    try {
      ...
      const rows = database
        .prepare(
          `SELECT id, ${columns.has('name') ? "COALESCE(NULLIF(name, ''), title) AS title" : 'title'}, cwd, updated_at, created_at, archived, source, ${columns.has('model') ? 'model' : 'NULL AS model'} FROM threads WHERE (cwd = ? OR cwd = ?) AND archived = 0 ${columns.has('agent_path') ? 'AND agent_path IS NULL' : ''} ORDER BY updated_at DESC LIMIT 250`
        )
        .all(cwd, path) as any[];
      return rows.map((row) => ({
        id: row.id,
  ...
  async sendMessage(
    ...
    const turns =
      history?.islands && history.entitiesByKey
        ? history.islands.flatMap((island: any) =>
            (island.entries ?? []).map((entry: any) => history.entitiesByKey[entry.value]).filter(Boolean)
          )
  ...
  async steer(
    ...
    const restoreMessage = {
      ...
      context: {
        ...
        workspaceRoots,
        collaborationMode:
          snapshot.state.latestCollaborationMode ?? snapshot.state.latestThreadSettings?.collaborationMode ?? null,
        addedFiles: [],
        fileAttachments: [],
        ...
        imageAttachments: images.map((image, index) => ({
          ...
          src: pathToFileURL(image.path).href,
    ...
    const response = await this.request(
      ...
      { conversationId: threadId, input, clientUserMessageId: clientMessageId, restoreMessage, attachments: [] },
      1,
      snapshot.ownerClientId,
      true
    );
  ...
  async interrupt(threadId: string, expectedTurnId: string): Promise<unknown> {
    if (!expectedTurnId) throw new NativeDesktopError('缺少要停止的原生轮次。', 'invalid_turn');
    const snapshot = await this.readThread(threadId);
    return (
      await this.request(
        'thread-follower-interrupt-turn',
        { conversationId: threadId, mode: 'user-stop', expectedTurnId },
        4,
        snapshot.ownerClientId,
        true
      )
  ...
  async respond(
    ...
    kind: NativeResponseKind,
    response: unknown
  ): Promise<unknown> {
    const snapshot = await this.readThread(threadId);
    const mapping = RESPONSE_METHODS[kind];
    if (!mapping) throw new NativeDesktopError('此原生请求请在 Codex App 中处理。', 'unsupported_request');
    const request = (snapshot.state.requests ?? []).find((item: any) => String(item.id) === String(requestId));
    if (!request || request.method !== mapping[1])
      throw new NativeDesktopError('该请求已处理或类型不匹配，请刷新对话。', 'stale_request');
    return (
      await this.request(
        mapping[0],
        {
          ...
          ...(['command', 'file'].includes(kind) ? { decision: response } : { response }),
        },
        1,
        snapshot.ownerClientId,
        true
  ...
  close(): void {
    ...
    for (const [id, sub] of this.subscriptions) this.follow(id, sub, false);
    this.socket?.destroy();
    for (const sub of this.subscriptions.values())
      this.failWaiters(sub, new NativeDesktopError('原生对话连接已关闭。'));
