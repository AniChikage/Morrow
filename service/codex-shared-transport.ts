...
function findSharedHosts(directory: string, codexHome: string): SharedHost[] {
  ...
  for (const name of readdirSync(directory).filter((name) => /^host-[0-9]+\.json$/.test(name))) {
    ...
    if (!privatePath(path, 'file')) continue;
    try {
      const host = JSON.parse(readFileSync(path, 'utf8'));
      if (
        host.version !== 1 ||
        typeof host.launchId !== 'string' ||
        typeof host.codexHome !== 'string' ||
        !samePath(host.codexHome, codexHome) ||
        !isAbsolute(host.socketPath) ||
        !alive(host.bridgePid) ||
        !alive(host.runtimePid) ||
        !alive(host.appPid)
      )
        continue;
      if (!privatePath(resolve(host.socketPath, '..'), 'directory') || !privatePath(host.socketPath, 'socket'))
        continue;
      hosts.push(host);
    } catch {
      /* Ignore stale or incomplete receipts from a previous launch. */
    }
  }
  return hosts;
}
export function findSharedHost(directory: string, codexHome: string): SharedHost | null {
  const hosts = findSharedHosts(directory, codexHome);
  if (hosts.length > 1)
    throw new NativeDesktopError('发现多个 Codex 后台，尚无法确认原生任务所属后台。', 'ambiguous_host');
  return hosts[0] || null;
}
...
interface Options {
  ...
  preferredThreadIds?: () => string[];
  onConnected?: (host: SharedHost) => void;
...
const userInput = (text: string, images: Array<{ path: string }>) => {
  ...
  if (images.some((image) => !isAbsolute(image.path) || image.path.includes('\0')))
    throw new NativeDesktopError('图片必须使用本地绝对路径。', 'invalid_image');
  return [
...
export class CodexSharedTransport {
  ...
  status() {
    return {
      connected: this.initialized && this.socket?.readyState === WebSocket.OPEN,
  ...
  threadStatus(id: string) {
    ...
    return {
      ready: this.status().connected && !!snapshot && !this.loading.has(id),
      detail: this.error || (snapshot ? '已连接 Codex App 共享后台。' : '正在恢复原生任务。'),
      ...(snapshot ? { lastSyncedAt: snapshot.syncedAt } : {}),
    };
  }
  ...
  private async selectHost(): Promise<SharedHost | null> {
    ...
    const hosts = findSharedHosts(
      this.options.directory || join(homedir(), 'Library/Application Support/Morrow/codex-bridge'),
      this.options.codexHome || process.env.CODEX_HOME || join(homedir(), '.codex')
    );
    const preferred = hosts.find((host) => host.launchId === this.options.preferredLaunchId?.());
    ...
    if (known.size) {
      ...
      const probes = await Promise.allSettled(
        hosts.map(async (host) => {
          const probe = new CodexSharedTransport({ host, timeoutMs: 5000 });
          try {
            return (await probe.loadedThreads()).some((id) => known.has(id));
          } finally {
            probe.close();
          }
        })
      ...
      const matches = hosts.filter(
        (_, index) => probes[index].status === 'fulfilled' && (probes[index] as PromiseFulfilledResult<boolean>).value
      );
      if (matches.length === 1 && probes.every((result) => result.status === 'fulfilled')) return matches[0];
    }
    throw new NativeDesktopError(
      '发现多个 Codex 后台，尚无法唯一确认已绑定任务所属后台；没有切换或恢复到其他后台。',
      'ambiguous_host'
    );
  }
  async loadedThreads(): Promise<string[]> {
    ...
    do {
      const page = await this.request('thread/loaded/list', { limit: 100, ...(cursor ? { cursor } : {}) });
      ids.push(...page.data);
      cursor = page.nextCursor || undefined;
      if (cursor && (seen.has(cursor) || seen.size >= 100))
        throw new NativeDesktopError('原生已加载任务分页无效。', 'protocol_mismatch');
  ...
  private async open() {
    ...
    const socket = new WebSocket('ws://localhost/rpc', {
      createConnection: () => createConnection(this.host!.socketPath),
      perMessageDeflate: false,
      maxPayload: 256 * 1024 * 1024,
      handshakeTimeout: this.options.timeoutMs || 15_000,
    });
    this.socket = socket;
    ...
    try {
      ...
      const result = await this.request('initialize', {
        clientInfo: { name: 'morrow', title: 'Morrow', version: '0.4.3' },
        capabilities: { experimentalApi: true },
      });
      if (!samePath(result.codexHome, this.host.codexHome))
  ...
  private request(method: string, params: any, mutation = false): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      if (this.socket?.readyState !== WebSocket.OPEN) {
        reject(new NativeDesktopError('原生后台未连接。'));
        return;
      }
      const timer = setTimeout(() => {
        ...
        reject(
          new NativeDesktopError(
            mutation ? '后台操作结果尚未确认，请等待同步，不要重复发送。' : '原生后台响应超时。',
            'request_timeout',
            mutation
          )
        );
      ...
      this.socket.send(JSON.stringify({ id, method, params }), (error) => {
        if (error) {
          clearTimeout(timer);
          this.pending.delete(id);
          reject(new NativeDesktopError(error.message, 'delivery_unknown', mutation));
        }
      });
    });
  }
  private receive(message: any) {
    ...
    if (message.id != null && !message.method) {
      ...
      if (message.error)
        pending.reject(
          new NativeDesktopError(message.error.message || '原生请求失败。', String(message.error.code), false)
        );
      else {
        if (message.result && typeof message.result === 'object') this.responseSequences.set(message.result, sequence);
        pending.resolve(message.result);
      }
      return;
    }
    if (message.method === usageUpdatedNotification && message.id == null) {
      const reading = parseUsageReading(message.params);
      if (reading) this.options.onUsage?.(reading);
      return;
    }
  ...
  private emit(id: string, patches: any[], nativeEvent?: any) {
    ...
    const snapshot = {
      ...
      state: applyDesktopPatches(old.state, patches),
    };
  ...
  private apply(id: string, event: any) {
    ...
    if (method === 'thread/status/changed') set(['threadRuntimeStatus'], p.status);
      ...
      const old = state.turns[turnIndex];
      const turn = {
        ...old,
        ...p.turn,
        turnId: p.turn.id,
      ...
      patches.push({
        op: turnIndex < 0 ? 'add' : 'replace',
        path: ['turns', turnIndex < 0 ? state.turns.length : turnIndex],
        value: turn,
      });
      set(['threadRuntimeStatus'], { type: method === 'turn/started' ? 'active' : 'idle' });
    } else if (method === 'item/started' || method === 'item/completed') {
      if (turnIndex < 0) {
        set(['turns', state.turns.length], { turnId: p.turnId, id: p.turnId, status: 'inProgress', items: [p.item] });
      } else {
        const index = state.turns[turnIndex].items.findIndex((item: any) => item.id === p.item.id);
        patches.push({
          op: index < 0 ? 'add' : 'replace',
          path: ['turns', turnIndex, 'items', index < 0 ? state.turns[turnIndex].items.length : index],
          value: p.item,
        });
      }
    ...
    if (method === 'serverRequest/resolved')
      set(
        ['requests'],
        state.requests.filter((request: any) => String(request.id) !== String(p.requestId))
      );
  ...
  async readThread(id: string): Promise<NativeThreadSnapshot> {
    ...
    const operation = (async () => {
      ...
      do {
        ...
        for (const turn of page.data) hydratedAt.set(turn.id, this.responseSequences.get(page) || 0);
        cursor = page.nextCursor;
        if (cursor && seen.has(cursor)) throw new NativeDesktopError('原生历史分页游标重复。', 'protocol_mismatch');
        if (cursor) seen.add(cursor);
      } while (cursor);
      const snapshot: NativeThreadSnapshot = {
        threadId: id,
        ownerClientId: this.projectionOwner,
        revision: 0,
        syncedAt: new Date().toISOString(),
        state: {
          id,
          ...
          currentPermissions: {
            runtimeWorkspaceRoots: [result.cwd],
            approvalPolicy: result.approvalPolicy,
            sandboxPolicy: result.sandbox,
          },
          threadRuntimeStatus: result.thread.status,
          turns: turns.map((turn) => ({ ...turn, turnId: turn.id })),
          requests: [],
        },
      };
      this.snapshots.set(id, snapshot);
    ...
    try {
      ...
      for (const { message, sequence } of this.buffered.get(id) || []) {
        ...
        const representedByHistory =
          message.id == null && (message.method?.startsWith('item/') || message.method?.startsWith('turn/'));
  ...
  async subscribeChanges(id: string, listener: ChangeListener) {
    ...
    return () => {
      ...
      if (!group.size) this.listeners.delete(id);
    };
  }
  async subscribe(id: string, listener: (snapshot: NativeThreadSnapshot) => void) {
    return this.subscribeChanges(id, (snapshot) => listener(structuredClone(snapshot)));
  }
  /** Account rate-limit windows; any protocol error, timeout, disconnect or odd payload resolves to `undefined`. */
  async readUsage(): Promise<UsageReading | undefined> {
    try {
      await this.connect();
      return parseUsageReading(await this.request(usageReadMethod, {}));
    } catch {
      return undefined;
    }
  }
  async listThreads(cwd: string) {
    await this.connect();
    const result = await this.request('thread/list', { cwd, limit: 250, archived: false, sortKey: 'updated_at' });
    return result.data.map((thread: any) => ({
      id: thread.id,
  ...
  async createThread(cwd: string): Promise<NativeThreadSnapshot> {
    ...
    await this.connect();
    const result = await this.request('thread/start', { cwd, ephemeral: false }, true);
    const snapshot: NativeThreadSnapshot = {
  ...
  async sendMessage(
    ...
    if (active && workOptions)
      throw new NativeDesktopError('原生任务已开始新的轮次，自动工作将在空闲后继续。', 'thread_busy');
    ...
    if (workOptions) {
      ...
      const state = this.snapshots.get(id)!.state;
      this.emit(id, [
        { op: 'add', path: ['latestThreadSettings'], value: { ...state.latestThreadSettings, ...workOptions } },
        { op: 'add', path: ['currentPermissions'], value: { ...state.currentPermissions, ...workOptions } },
      ]);
    }
    return result;
  }
  async interrupt(id: string, turnId: string) {
    ...
    return this.request('turn/interrupt', { threadId: id, turnId }, true);
  }
  async respond(id: string, requestId: string | number, kind: string, value: unknown) {
    const snapshot = await this.readThread(id);
    const request = snapshot.state.requests.find((request: any) => String(request.id) === String(requestId));
    const matches =
      kind === 'command'
          ...
          : kind === 'permissions'
            ...
            : kind === 'userInput'
              ...
              : kind === 'mcp'
                ? 'elicitation/request'
                : '';
    if (!request || !matches || !request.method.includes(matches))
      throw new NativeDesktopError('原生请求已处理或类型不匹配。', 'stale_request');
    ...
    this.emit(id, [
      {
        op: 'replace',
        path: ['requests'],
        value: snapshot.state.requests.filter((item: any) => item.id !== request.id),
      },
    ]);
    return { ok: true };
  }
