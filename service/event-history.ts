...
function scope(store: Store, params: URLSearchParams) {
  ...
  if (channelId && (!channel || (projectId && channel.projectId !== projectId)))
    throw new APIError(404, '频道不属于该项目或不存在');
...
export function eventHistory(store: Store, params: URLSearchParams) {
  ...
  const itemId = queryID(params, 'itemId');
  const runId = queryID(params, 'runId');
  const before = queryID(params, 'before');
  const after = queryID(params, 'after');
  if (before && after) throw new APIError(400, 'before 与 after 不能同时使用');
  const matches = (row: { projectId?: string; channelId?: string; itemId?: string; runId?: string }) =>
    (!projectId || row.projectId === projectId) && (!channelId || row.channelId === channelId);
  if (runId) {
    const run = store.get<Run>('runs', runId);
    if (!run || !matches(run)) throw new APIError(404, '运行记录不属于查询范围或不存在');
  }
  if (itemId) {
    const item = store.get<WorkItem>('items', itemId);
    const project = projectId || store.get('channels', channelId!)?.projectId;
    if (!item || item.projectId !== project) throw new APIError(404, '事项不属于查询范围或不存在');
  }
  if (before || after) {
    const cursor = store.get<Event>('events', (before || after)!);
    if (!cursor || !matches(cursor) || (runId && cursor.runId !== runId) || (itemId && cursor.itemId !== itemId))
      throw new APIError(404, '事件游标不属于查询范围或不存在');
  }
...
export function runHistory(store: Store, params: URLSearchParams) {
  ...
  if (before && after) throw new APIError(400, 'before 与 after 不能同时使用');
  if (before || after) {
    const run = store.get<Run>('runs', (before || after)!);
...
export function runOutput(store: Store, runId: string, params: URLSearchParams) {
  ...
  queryFields(params, ['after', 'limit']);
  const after = queryID(params, 'after');
  if (after && store.get<RunIO>('run_io', after)?.runId !== runId)
    throw new APIError(404, '输出游标不属于本轮运行或不存在');
