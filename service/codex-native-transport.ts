...
export class CodexNativeTransport implements NativeTransport {
  ...
  subscriptions = new Set<{
    id: string;
    listener: (snapshot: NativeSnapshot, change: any) => void;
    stop?: () => void;
  }>();
  connecting: Promise<void> | null = null;
  disposed = false;
  constructor(desktop = new CodexDesktopTransport(), shared = new CodexSharedTransport()) {
  ...
  async connect() {
    ...
    this.connecting = (async () => {
      ...
      for (const subscription of this.subscriptions) {
        ...
        void this.attachSubscription(subscription, selected).catch(() => {
          /* The next read surfaces the native recovery error. */
        });
      }
    })().finally(() => {
  ...
  threadStatus(id: string) {
    return this.current.threadStatus?.(id) || { ready: false, detail: '正在连接原生任务。' };
  }
  async listThreads(cwd: string) {
    await this.connect();
    return this.current.listThreads(cwd);
  }
  async readThread(id: string) {
    await this.connect();
    return this.current.readThread(id);
  }
  async loadCompleteHistory(id: string) {
    await this.connect();
    return this.current.loadCompleteHistory?.(id) || this.current.readThread(id);
  }
  private async attachSubscription(
    subscription: { id: string; listener: (snapshot: NativeSnapshot, change: any) => void; stop?: () => void },
    transport: NativeTransport
  ) {
    const stop = await transport.subscribeChanges!(subscription.id, (snapshot, change) => {
      if (!this.disposed && this.current === transport && this.subscriptions.has(subscription))
        subscription.listener(snapshot, change);
    });
    if (this.disposed || this.current !== transport || !this.subscriptions.has(subscription)) stop();
    else subscription.stop = stop;
  }
  async subscribeChanges(id: string, listener: (snapshot: NativeSnapshot, change: any) => void) {
    await this.connect();
    const subscription = { id, listener, stop: undefined as (() => void) | undefined };
    this.subscriptions.add(subscription);
    try {
      await this.attachSubscription(subscription, this.current);
    } catch (error) {
  ...
  async createThread(cwd: string) {
    ...
    if (!this.backgroundReady)
      throw new NativeDesktopError(
        '完成一次 Codex 后台连接设置后，即可直接在这里新建对话。',
        'shared_host_unavailable'
      );
    return this.shared.createThread(cwd);
  }
  async sendMessage(
    id: string,
    text: string,
    ...
    await this.connect();
    if (workOptions && !this.backgroundReady)
      throw new NativeDesktopError(
        '自动工作需要 Codex 共享后台连接，以应用原生自动审查和频道工作范围。',
        'shared_host_unavailable'
      );
  ...
  async interrupt(id: string, turnId: string) {
    await this.connect();
    return this.current.interrupt(id, turnId);
  }
  async respond(
    id: string,
    ...
    await this.connect();
    return this.current.respond(id, requestId, kind, response);
  }
  close() {
    this.disposed = true;
    for (const item of this.subscriptions) item.stop?.();
