...
export type ActionOption = {
  title: string;
  kind: 'act' | 'investigate' | 'build_capability' | 'observe' | 'stop';
  benefit: string;
  cost: string;
  uncertainty: string;
};
...
export type MemoryMatch = {
  ...
  caution?: string;
  reviewAt?: string;
  outcome?: 'improved' | 'not_improved' | 'inconclusive' | 'abandoned';
  reasons: string[];
};
export type MemoryReference = {
  ...
  id: string;
  revision: number;
  use: 'apply' | 'adapt' | 'avoid' | 'not_applicable';
  reason: string;
...
export type ObservationStatus = {
  ...
  status: 'unplanned' | 'waiting' | 'needs_repair' | 'ready';
  evidenceId?: string;
  baselineEvidenceId?: string;
  observedValue?: string | number | boolean | null;
  baselineValue?: string | number | boolean | null;
  comparedValue?: string | number | boolean | null;
...
export type ExpectationResult = {
  ...
  reason: string;
  evidenceIds: string[];
  checkedBy: 'rule' | 'agent';
  observedValue?: string | number | boolean | null;
  observation?: ObservationStatus;
};
export type DecisionAssessment = {
  results: ExpectationResult[];
  conditions: 'matched' | 'changed' | 'unknown';
...
export type DecisionView = StrategyDecision & {
  runsUsed: number;
