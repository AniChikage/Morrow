...
export class ProjectWorkLoop {
  ...
  view(projectId:string,itemId?:string):ProjectLoop {
    ...
    const references=new Set([...learning.flatMap(row=>row.evidenceIds),...releases.flatMap(row=>row.checks.flatMap(check=>check.evidenceIds)),...strategy.understanding.flatMap(row=>row.evidenceIds),...strategy.decisions.flatMap(row=>[...row.evidenceIds,...(row.expectations||[]).flatMap(e=>e.measurement&&'evidenceId' in e.measurement.baseline?[e.measurement.baseline.evidenceId]:[]),...(row.review?.evidenceIds||[]),...(row.memoryRefs||[]).flatMap(ref=>ref.snapshot.evidenceIds)])]);
  ...
  context(scope:Scope,input:Record<string,any>) {
    ...
    return {project:{id:project.id,name:project.name,goal:project.goal},channel:{id:channel.id,goal:channel.goal},strategy:this.strategy.context(scope,item?.id),learningCoverage:{total:learningTotal,included:learning.length,partial:learningTotal>learning.length,readMore:'这里只列最近 12 条摘要；历史和全文用 memory.recall/search/read 读取。'},features:item?[item]:this.store.projectItems(project.id),learning,evidence,watches:view.watches,releases:view.releases,verifications:view.verifications,finalizations:view.finalizations,executions:this.rows<any>('loop_executions',project.id).slice(-12),operations:{
      ...
      'observation.read':'{decisionId}；只读核对原始计划、基线、最新窗口内观测与数据质量，返回 observations（ready/waiting/needs_repair/unplanned）、系统 verdict、原值/基线/比较值及具体 issues；context.strategy.decisions 同时包含此状态。仅为规则核对，不代表独立复核或业务因果证明。缺数据先调查、建立观测能力或等待；检查通过才按实际结果复盘。',
      ...
      'execution.prepare':'{command:实际原生命令的完整字符串}；测试/构建前调用，框架封存当前源版本。随后在同一原生轮次、项目根目录执行完全相同命令。Morrow 直接从原生事件保存命令、输出、退出码、任务/轮次和执行前后源版本；execution.read 读取。不要用自行生成的 JSON 或 package.json 证明执行成功。',
  ...
  async poll(id:string){
    ...
    try{const {data,hash}=await jsonRequest(watch.url);const value=valueAt(data,watch.pointer);if(value===undefined)throw new Error('数据中不存在指定字段');const valueHash=digest(JSON.stringify(value));const changed=watch.lastDigest!==valueHash;let evidenceId=watch.lastEvidenceId;
      ...
      this.store.put('loop_watches',{...current,lastValue:value,lastDigest:valueHash,lastEvidenceId:evidenceId,error:undefined,status:current.status==='watching'?(met?'triggered':'watching'):current.status,updatedAt:now(),nextPollAt:new Date(Date.now()+watch.intervalSeconds*1000).toISOString()});if(watch.error)this.signal(watch.channelId,id,`反馈来源已恢复：${watch.title}`);if(current.status==='watching'?met:!!watch.lastDigest&&changed)this.signal(watch.channelId,id,`收到新的反馈：${watch.title}`);
