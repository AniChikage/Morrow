...
try {
  ...
  if (!contextPath) throw new Error('缺少 --context');
  ...
  if (
    ...
    throw new Error('写操作需要稳定的 --request-id；重试必须沿用相同 ID 和输入');
  const response = await fetch(context.url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${context.token}` },
    body: JSON.stringify({ operation, input, ...(requestId ? { requestId } : {}) }),
  ...
  process.stderr.write((error instanceof Error ? error.message : 'Morrow 工具调用失败') + '\n');
  process.exitCode = 1;
}
