...
export function importNativeImages(store: Store, home: string, channelId: string, paths: string[]) {
  ...
  if (!Array.isArray(paths) || paths.length < 1 || paths.length > 5)
    throw new APIError(400, '一次请选择 1 至 5 张图片');
  const images = paths.map((path) => ({ name: basename(path), bytes: readImage(path) }));
  if (images.reduce((sum, image) => sum + image.bytes.length, 0) > MAX_BATCH)
    throw new APIError(413, '一批图片总大小最多 20 MB');
  ...
  return images.map(({ name, bytes }) => {
    const id = randomUUID(),
      ...
      path = join(directory, `${id}.${type.extension}`);
    ...
    const row: Attachment = {
      ...
      mimeType: type.mime,
      path,
      dataUrl: `data:${type.mime};base64,${bytes.toString('base64')}`,
...
export function resolveNativeAttachments(
  ...
  if (
    ...
    new Set(attachments.map((value) => value?.id)).size !== attachments.length
  )
    throw new APIError(400, '图片附件列表无效');
  return attachments.map((value) => {
    ...
    if (createHash('sha256').update(bytes).digest('hex') !== row.sha256)
      throw new APIError(409, '图片附件已改变，请重新添加');
    return { path: row.path };
  });
}
export function readNativeImage(store: Store, channelId: string, itemId: string, index: number): { dataUrl: string } {
  ensureChannel(store, channelId);
  const binding = store.get('native_bindings', channelId),
    item = store.get('native_items', itemId);
  if (
    !binding ||
    !item ||
    ...
    throw new APIError(404, '图片不属于当前原生对话');
  ...
  const key = createHash('sha256')
    .update(`${channelId}:${itemId}:${index}:${JSON.stringify(part)}`)
    .digest('hex');
  ...
  if (typeof source === 'string' && source.startsWith('data:')) {
    ...
    if (!match || match[1].length > Math.ceil(MAX_IMAGE / 3) * 4)
      throw new APIError(413, '图片内容格式无效或超过 10 MB');
    bytes = Buffer.from(match[1], 'base64');
  } else if (part.type === 'localImage' && typeof source === 'string') {
    try {
      bytes = readImage(source);
    } catch (error) {
      if (error instanceof APIError) throw error;
      throw new APIError(404, '原生图片文件暂时不可用，请在 Codex App 中查看');
    }
  } else throw new APIError(409, '此图片由 Codex App 托管，请在 App 中查看');
  const type = imageType(bytes),
    dataUrl = `data:${type.mime};base64,${bytes.toString('base64')}`;
  store.put('native_attachments', {
    id: key,
    channelId,
    name: '原生图片',
    mimeType: type.mime,
