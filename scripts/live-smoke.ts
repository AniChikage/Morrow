...
if (!process.argv.includes('--run')) throw new Error('Use --run to invoke real authenticated CLI runtimes.');
...
try {
  await Promise.all(
    service.engine.runtimes.map(async (runtime) => {
      const filter = process.argv.find((arg) => arg.startsWith('--runtime='))?.split('=')[1];
      ...
      mkdirSync(dir);
      const source = 'export function ratio(done, total) { return done / total; }\n';
      ...
      const channel = service.store.all<any>('channels').find((c) => c.projectId === project.id);
      await api(
        `channels/${channel.id}`,
        {
          runtime: runtime.id,
      ...
      while (Date.now() - began < 150000) {
        const run = service.store.all<any>('runs').find((r) => r.channelId === channel.id);
        if (run && run.status !== 'running') break;
        await new Promise((resolve) => setTimeout(resolve, 500));
      }
      if (service.engine.active.has(channel.id)) await api(`channels/${channel.id}/action`, { action: 'pause' });
      ...
      const report = {
        ...
        status: run?.status,
        summary: run?.summary,
        findings: service.store.all<any>('items').filter((i) => i.channelId === channel.id).length,
        fixtureUnchanged: intact,
        seconds: Math.round((Date.now() - began) / 1000),
      };
      reports.push(report);
      console.log(JSON.stringify(report));
      if (!intact) throw new Error(`${runtime.id}: read-only fixture was modified`);
    })
  );
} finally {
  await service.close();
