...
通过 CUA 操作实际 `.app` 验证：首次启动自动连接、加载明确示例、项目/频道切换、证据详情、中文搜索、配置保存、运行引擎状态、连接设置、新建真实 NoHuman 项目。

最终真实项目路径为 `/Users/bytedance/workspace/nohuman`，两个频道均为暂停状态。UI 测试未启动真实项目的执行。
