...
Claude uses `--print --verbose --output-format stream-json` and an exact `--resume` ID when present, without forced `--json-schema`. It retains `--safe-mode`, strict empty MCP configuration (`--strict-mcp-config --mcp-config {"mcpServers":{}}`), an explicit session name (`--name Morrow:<runId>`) and explicit tool lists stated twice, as `--tools` and `--allowedTools`: Read/Grep/Glob with `--permission-mode dontAsk` for read-only; plus Edit/Write/MultiEdit/NotebookEdit/Bash with `--permission-mode acceptEdits` for workspace editing. Workspace editing therefore runs commands, and those commands are not sandboxed by Morrow. `--safe-mode` loads no project CLAUDE.md, hooks, plugins, skills or MCP servers, so native login and session reuse do not imply unchanged inheritance of Claude custom configuration, plugins or tools.
