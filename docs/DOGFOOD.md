...
4. 发布前做一次发布级复核（`verification.request kind:release`），事项只需各自改动时的复核通过；源码再变更时只需重做发布级复核。走 App 任务时引用本次候选上真实跑过的完整检查（先 `execution.prepare` 再执行，取绑定当前源版本的 execution 证据）；CLI 频道改为 file/http 采集证据，由独立复核者在隔离检出里重跑检查。事项自身的完成仍要求该事项当前源码版本的复核。
