...
Opening a project starts from an existing folder and a default native runtime. Native handoff opens the exact saved session or an interactive CLI in that folder; it requires the entire project to be paused and local. Native authentication and session history stay with the CLI. NoHuman schedules bounded rounds and mirrors the I/O it manages; manually continued terminal sessions are not silently imported.
...
- `codesign --verify --deep --strict dist/NoHuman.app`: passed.
- Final `hdiutil verify dist/NoHuman.dmg`: checksum valid (190,555,700 bytes).
...
An online SQLite backup was created at `~/Library/Application Support/NoHuman/backups/product-v030-20260907T041607Z/`. Pre-upgrade counts: 2 projects, 4 paused channels, 5 items, 0 runs, 6 events; no enabled schedules. No user records were changed during tests.
...
Installation completed later on 2026-09-07 after the Mac was unlocked. A new online backup was created at `~/Library/Application Support/NoHuman/backups/install-v030-20260907T060046Z/`. The installer preserved the previous application bundle, and installed `~/Applications/NoHuman.app` version 0.3.0. The installed asar and all eight service source files match the build byte-for-byte; signature verification passed.
