...
新频道默认「沿用 Codex App 原生权限」：自动轮次不再附加 Morrow 自己的沙箱或断网设置，只沿用原生自动审查处理审批；Codex 在自动轮次里拥有与你在 App 中相同的权限（App 默认为完整访问，可修改整个项目、联网、使用浏览器等原生工具）。这是单人使用下的有意选择，人工合并与上线确认是仅有的两道闸。
...
早期版本支持过 Claude Code 与 Trae。它们的频道、运行记录和事件保持可读，界面标记为「已停止支持」；这些频道不再调度，不能手动运行或继续，也不能打开原生会话。服务巡检发现仍在启用的旧频道时，会关闭其调度、置为暂停并记录一条系统事件。新工作请在同一项目下新建 Codex 频道。
