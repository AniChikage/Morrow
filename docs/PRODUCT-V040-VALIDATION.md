...
本轮连接正在运行的 Codex App 任务 `01a07725-b183-70d0-a9e3-2447b54f5f71`，在 Morrow 项目的「系统完善」频道关联该任务。未启动独立 Codex CLI 会话。

- App 端输入和助手回复从原生历史进入 Morrow，包含当前轮次的追加消息。
- 通过 Morrow 服务发送 `MORROW-20260907-2` 后，消息实际进入当前 App 任务；收到 `SYNC-MORROW-OK`。重连后用户消息只有一条，原生轮次与发件回执一致。
- 从已安装 App 的输入框发送 `MORROW-UI-20260907`，当前 App 任务实际收到并回复 `SYNC-UI-OK`。数据库中用户消息和对应确认回复各一条，回执为 accepted，附件数为零。
...
安装位置为 `~/Applications/Morrow.app`。升级前做 SQLite 在线备份并保留旧 App；核对原有 3 个项目、7 个频道、5 个看板事项、旧运行和事件全部保留。原生历史导入产生额外运行记录，未触发项目自动执行。
...
最终重启后仍连接同一原生任务与轮次，验收用户消息和回复各一条，所有原有记录保留。[最终安装状态](../artifacts/product-v040/final-install-state.json)、[最终对话界面](../artifacts/product-v040/installed-native-chat-final.jpg)。最终包为 `dist/Morrow.app` 与 `dist/Morrow.dmg`。

## 验收范围

实机覆盖本机现有任务的历史读取、活动轮次追加、双向文字、重连和安装。空闲任务的新轮次、审批问答、实际图片发送、自动职责完成与看板更新有协议/模拟回归，本轮未触发这些真实模型操作。

当前实现依赖安装版 Codex App 的私有本机 IPC；协议升级不兼容时显示连接错误并禁用发送。创建新任务需先打开 App 的项目输入框、发送首条消息，再回 Morrow 关联。远程 App 同步及 App 专属功能不在本轮支持范围。构建为本机 ad-hoc 签名，跨设备正式分发仍需 Developer ID 和公证。
