...
- **复核工作进程 `codex-cli-worker.ts`**（`service/codex-cli-review.ts` 按自己的模块路径启动它，只在一次独立复核开始时）：**仍在窗口内**。如果新版本改了它与 `codex-cli-review.ts` 之间的 IPC 消息形状，切换完成前开始的那次复核可能失败或中途停下；复核失败/未知都有既有处理（保留 `unknown`、可 `verification.retry`），不会写出虚假结论。把它一起钉住需要改协议层的那个启动点。
