...
1. **打开项目文件夹**，选择现有目录，写下持续目标。新项目会准备一个暂停的「自主推进」频道，默认沿用 Codex App 的权限设置（以 App 中当前的设置为准；单人使用时建议在 App 里选择完整访问），每日最多 32 轮；接入本身不会启动执行。需要收紧时，可在频道设置中改为只读或工作区写入。
2. **设置 Codex 后台连接**。打开「运行时」页，按四步清单核对：Codex App 已安装 → App 原生后台运行中 → 后台桥接已配置 → 桥接已生效；清单下方只给出一个下一步，后台连接就在这里启用或撤销。首次配置后，在当前任务结束时重新打开一次 Codex App。之后可直接在 Morrow 创建原生任务，或关联同一项目已有任务；无需每次在 Codex App 中打开对应页面。
...
- [原生运行时](RUNTIMES.md)：Codex 连接要求、默认权限与能力边界，以及旧 Claude Code / Trae 记录的处理。
