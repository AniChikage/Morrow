...
export class FakeReviewer implements NativeTransport {
  ...
  async createThread(cwd: string) {
    const threadId = randomUUID(),
      snapshot: NativeSnapshot = {
        threadId,
        ownerClientId: 'fake-reviewer',
        revision: 0,
        syncedAt: new Date().toISOString(),
        state: { cwd, model: 'fake-review-model', turns: [] },
      };
  ...
  async subscribe(id: string, listener: (s: NativeSnapshot) => void) {
  ...
  complete(
    id = this.sent.at(-1)!.threadId,
    options: { report?: unknown; command?: boolean; status?: string; items?: any[] } = {}
  ) {
    const snapshot = this.snapshots.get(id)!,
      turn = snapshot.state.turns[0];
    turn.status = options.status || 'completed';
    turn.items = options.items || [
        ...
        : [
            {
              ...
              command: 'read-only fixture check',
              cwd: snapshot.state.cwd,
              status: 'completed',
              exitCode: 0,
              aggregatedOutput: 'fixture inspected',
            },
          ]),
      {
        id: 'final',
  ...
  async interrupt(threadId: string, turnId: string) {
    ...
    const s = this.snapshots.get(threadId);
    if (s) s.state.turns[0].status = 'interrupted';
    return {};
  }
  async respond() {
    throw new Error('reviewer must not approve requests');
  }
  close() {}
}
