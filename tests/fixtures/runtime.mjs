...
let config = {};
try {
  config = JSON.parse(readFileSync(join(process.cwd(), '.fixture.json'), 'utf8'));
} catch {}
writeFileSync(join(process.cwd(), '.fixture-capture.json'), JSON.stringify({ args, input, pid: process.pid }));
if (config.sleep) {
  const child = spawn(
    process.execPath,
    [
  ...
  console.log(JSON.stringify({ type: 'thread.started', thread_id: 'fixture-session-1' }));
  setInterval(() => {}, 1000);
  ...
  console.error(
    config.authFailure ? 'get_detail_param returned 401; authentication_failed' : 'Fixture provider failure'
  );
  process.exit(2);
