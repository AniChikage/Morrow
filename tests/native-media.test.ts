...
const png = Buffer.from(
  'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jTfoAAAAASUVORK5CYII=',
  'base64'
);
function fixture(t: any) {
  const home = mkdtempSync(join(tmpdir(), 'morrow-media-')),
    store = new Store(join(home, 'workspace.sqlite'));
  t.after(() => {
    store.db.close();
  ...
  const path = join(home, 'chosen.png');
...
test('selected native images persist immutable private copies and full database content', (t) => {
  const { home, store, path } = fixture(t),
    [image] = importNativeImages(store, home, 'channel', [path]);
  rmSync(path);
...
test('native images are scoped to the current bound item and cached for offline history', (t) => {
  ...
  store.put('native_items', {
    ...
    threadId: 'thread',
    present: true,
    raw: { type: 'steeringUserMessage', input: [{ type: 'image', image_url: first.dataUrl }] },
  });
