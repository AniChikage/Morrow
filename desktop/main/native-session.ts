...
function absolutePath(value: unknown, label: string): asserts value is string {
  if (typeof value !== 'string' || value.length > 4096 || !isAbsolute(value) || /[\0\r\n]/.test(value))
    throw new Error(`${label}必须是有效的本机绝对路径。`);
}
function validate(target: NativeSessionTarget): void {
  if (
    ...
    Object.keys(target).some((key) => !['projectPath', 'runtime', 'executable', 'sessionId'].includes(key))
  )
...
export async function launchNativeSession(target: NativeSessionTarget): Promise<void> {
  ...
  try {
    ...
    if (!info.isDirectory() || info.isSymbolicLink() || (info.mode & 0o077) !== 0)
      throw new Error('unsafe launch directory');
  } catch {
    throw new Error('无法创建私有原生会话启动目录。');
  }
  const file = join(directory, `${target.runtime}-${randomUUID()}.command`);
  try {
    await writeFile(file, script, { mode: 0o700, flag: 'wx' });
  } catch {
    throw new Error('无法保存原生会话启动文件。');
  }
  let error: string;
  try {
    error = await shell.openPath(file);
  } catch {
    throw new Error('无法打开终端原生会话，请检查 .command 文件的默认打开方式。');
  }
