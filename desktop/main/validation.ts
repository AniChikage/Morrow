...
export function connectionConfig(value: unknown): ConnectionConfig {
  ...
  if (
    mode === 'ssh' &&
    (!/^[A-Za-z0-9][A-Za-z0-9_.@:-]*$/.test(host) || !/^(\/|~\/)/.test(directory) || /[\r\n]/.test(directory))
  ) {
...
export function channelPatch(value: unknown): ChannelPatch {
  ...
  if (data.permission !== undefined)
    result.permission = choice(data.permission, ['read-only', 'workspace-write', 'native']);
...
export function itemInput(value: unknown): CreateItem {
  ...
  return {
    ...
    title: result.title,
    ...(channelId !== undefined ? { channelId: channelId === '' ? '' : id(channelId) } : {}),
  };
}
function pageFields(data: Record<string, unknown>): RunsQuery {
  const result: RunsQuery = {};
  for (const key of ['projectId', 'channelId', 'before', 'after'] as const)
    if (data[key] !== undefined) result[key] = id(data[key]);
...
export function runOutputInput(value: unknown): RunOutputQuery {
  ...
  return {
    ...(data.after !== undefined ? { after: id(data.after) } : {}),
    ...(data.limit !== undefined ? { limit: integer(data.limit, 1, 100) } : {}),
  };
}
export function nativeMessageInput(value: unknown): NativeMessageInput {
  const data = record(value, ['text', 'requestId', 'attachments']);
  if (data.attachments !== undefined && (!Array.isArray(data.attachments) || data.attachments.length > 5))
    throw new Error('一次最多发送 5 张图片。');
  const attachments = (data.attachments as unknown[] | undefined)?.map((value) => {
    const entry = record(value, ['id', 'name', 'mimeType']);
    return {
      id: id(entry.id),
      name: text(entry.name, '图片名称', 255),
      mimeType: choice(entry.mimeType, ['image/png', 'image/jpeg', 'image/webp', 'image/gif']),
    };
  });
...
export function externalURL(value: unknown): string {
  ...
  if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password)
    throw new Error('只能打开 HTTP 或 HTTPS 网页链接。');
  return url.href;
}
