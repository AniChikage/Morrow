...
export class ServiceConnection {
  ...
  private async initializeOnce(): Promise<void> {
    ...
    try {
      config = connectionConfig(JSON.parse(await readFile(this.preferencesPath, 'utf8')));
    } catch {
      config = await this.importSwiftConnection();
    }
  ...
  info(): ConnectionInfo {
    return {
      ...
      name: this.config.mode === 'ssh' ? `远程 · ${this.config.host}` : '本机 Mac',
      ...(this.error ? { error: this.error } : {}),
    };
  }
  async getInfo(): Promise<ConnectionInfo> {
  ...
  private async importSwiftConnection(): Promise<ConnectionConfig> {
    ...
    const preference = async (domain: string, key: string): Promise<string> => {
      try {
        return (
          await execute('/usr/bin/defaults', ['read', domain, key], { timeout: 2000, maxBuffer: 8192 })
        ).stdout.trim();
      } catch {
        return '';
      }
    ...
    try {
      return connectionConfig({
        mode: 'ssh',
        host,
        port: Number(port || 43821),
        directory: directory || '~/.local/share/morrow',
      });
    } catch {
      return this.config;
    }
  ...
  private async switchConnection(config: ConnectionConfig, persist: boolean): Promise<ConnectionInfo> {
    ...
    this.config =
      config.mode === 'local'
        ? { mode: 'local', host: '', port: this.localPort, directory: this.dataDirectory }
        : config;
  ...
  private async probe(port: number): Promise<'online' | 'absent' | 'other'> {
    try {
      ...
      return response.ok && value.ok === true && ['morrow', 'nohuman'].includes(value.service || '')
        ? 'online'
        : 'other';
    } catch (error) {
  ...
  private async ensureLocalService(): Promise<void> {
    ...
    const candidates = app.isPackaged
      ? [join(process.resourcesPath, 'bin/node')]
      : [
          join(projectRoot, '.build/runtime-cache/node/bin/node'),
          process.env.MORROW_NODE || process.env.NOHUMAN_NODE || '',
    ...
    const entry = app.isPackaged
      ? join(process.resourcesPath, 'service/server.ts')
      : join(projectRoot, 'service/server.ts');
    await access(entry);
    await mkdir(this.dataDirectory, { recursive: true, mode: 0o700 });
  ...
  private async startTunnel(config: ConnectionConfig): Promise<void> {
    ...
    const tokenPath = config.directory.startsWith('~/')
      ? `"$HOME"/${quote(`${config.directory.slice(2)}/token`)}`
      : quote(`${config.directory}/token`);
    let token = '';
    try {
      const result = await execute(
        ...
        [
          ...
          'ConnectTimeout=8',
          config.host,
          `head -c 129 -- ${tokenPath}`,
        ],
        { timeout: 12000, maxBuffer: 1024 }
      );
      token = result.stdout.trim();
    } catch {
      throw new Error('无法读取远程服务令牌。请先在终端完成 SSH 首次连接，并确认远程服务已运行。');
    }
    ...
    const child = spawn(
      ...
      [
        ...
        '-L',
        `127.0.0.1:${this.tunnelPort}:127.0.0.1:${config.port}`,
        config.host,
      ],
      { stdio: 'ignore' }
    );
    ...
    child.once('exit', () => {
      if (this.tunnel === child) {
        this.connected = false;
        this.remoteToken = '';
        this.error = 'SSH 连接已断开，请重新连接。';
      }
    });
    await delay(700);
    if (child.exitCode !== null || child.signalCode !== null)
      throw new Error('SSH 隧道未能建立，请检查主机连接或本机 43822 端口。');
  ...
  private safeError(error: unknown): string {
    if (error instanceof ServiceError && error.status === 401)
      return '执行服务拒绝认证，请检查数据目录是否属于当前服务。';
  ...
  private async requestDirect<T>(path: string, method = 'GET', body?: unknown): Promise<T> {
    ...
    const token =
      this.config.mode === 'ssh'
        ? this.remoteToken
        : (await readFile(join(this.dataDirectory, 'token'), 'utf8')).trim();
    ...
    try {
      ...
      let events = snapshot.events.filter(
        (event) =>
          ...
          (!query.projectId ||
            (event.projectId || snapshot.channels.find((channel) => channel.id === event.channelId)?.projectId) ===
              query.projectId) &&
