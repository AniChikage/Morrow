...
export interface Run { projectId?: string; model?: string; permission?: Channel['permission'] | 'native'; executionOwner?: 'codex-app' | 'codex-cli' | 'cli'; source?: 'morrow-schedule' | 'morrow-chat' | 'nohuman-schedule' | 'nohuman-chat' | 'native-app' | 'native-cli'; nativeSettings?: Record<string, unknown>; trigger?: 'manual' | 'schedule'; resumedFromSessionId?: string; reportStatus?: 'pending' | 'valid' | 'missing' | 'invalid' | 'conflict'; reportError?: string; exitCode?: number; signal?: string; id: string; channelId: string; runtime: string; status: string; startedAt: string; finishedAt: string; summary: string; sessionId: string }
