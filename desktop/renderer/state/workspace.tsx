...
const unavailable = async (): Promise<never> => {
  throw new Error('桌面连接不可用，请重新打开 Morrow。');
};
...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const fetchSnapshot = useCallback(
    (force = false): Promise<void> => {
      ...
      const isCurrent = () =>
        mounted.current && currentGeneration === generation.current && number === requestNumber.current;
      const task = (async () => {
        ...
        try {
          data = await desktopAPI.getState();
        } catch (failure) {
        ...
        if (!isCurrent()) return;
        try {
          info = await desktopAPI.getConnection();
        } catch (failure) {
          infoFailure = failure;
        }
        if (!isCurrent()) return;
