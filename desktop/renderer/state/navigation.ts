...
interface ScopedSession extends Session {
  scope: string | null;
  hydrated: boolean;
}
...
function validRoute(value: unknown): value is Route {
  ...
  return (
    ['runs', 'runtimes'].includes(route.kind) ||
    (['project', 'finding', 'channel'].includes(route.kind) &&
      'id' in route &&
...
function readSession(scope: string): Session {
  try {
    ...
    if (
      ...
      new Set(value.tabs.map((tab: TabSession) => tab.id)).size === value.tabs.length &&
      (value.tabs.length ? value.tabs.some((tab: TabSession) => tab.id === value.active) : value.active === '')
    ) {
      // An explicitly saved empty session means the user closed their last tab.
      return { navigation: value, initialized: true };
    }
  } catch {
    /* Invalid or unavailable storage starts a fresh in-memory session. */
  }
  return { navigation: emptyNavigation(), initialized: false };
}
function newTab(route: Route): TabSession {
  return { id: crypto.randomUUID(), history: [route], index: 0 };
}

export function useNavigation(scope: string | null) {
  const [session, setSession] = useState<ScopedSession>({
    scope: null,
    hydrated: false,
    initialized: false,
  ...
  const commit = useCallback(
    (update: (previous: Session) => Session) => {
  ...
  const save = useCallback(
    (update: (previous: Navigation) => Navigation) =>
      commit((previous) => {
  ...
  const navigate = useCallback(
    (route: Route, openInNewTab = false) =>
      save((previous) => {
        ...
        return {
          ...
          tabs: previous.tabs.map((tab) =>
            tab.id === active.id
              ? {
                  ...tab,
                  history: [...tab.history.slice(0, tab.index + 1), route].slice(-40),
                  index: Math.min(tab.index + 1, 39),
                }
              : tab
          ),
        };
  ...
  const close = useCallback(
    (id: string) =>
      save((previous) => {
        ...
        return {
          tabs,
          active: previous.active === id ? tabs[Math.min(index, tabs.length - 1)]?.id || '' : previous.active,
        };
      }),
    [save]
  );
  ...
  const activate = useCallback(
    (id: string) =>
      save((previous) =>
        id === previous.active || !previous.tabs.some((tab) => tab.id === id) ? previous : { ...previous, active: id }
      ),
    [save]
  );
  ...
  return {
    hydrated,
    needsDefault: hydrated && !session.initialized,
    tabs: navigation.tabs,
    activeId: navigation.active,
    route: active?.history[active.index],
    canBack: !!active && active.index > 0,
    canForward: !!active && active.index < active.history.length - 1,
    navigate,
    openDefault,
    close,
    activate,
