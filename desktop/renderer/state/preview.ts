...
const channels: Channel[] = [
  ...
  },
  {
    ...
    projectId: 'demo-atlas',
    name: '运营洞察',
    goal: '发现有证据支持的用户体验与增长机会，区分假设和事实。',
    // The preview shows both execution paths, matching the demo project the service seeds.
    runtime: 'claude',
    model: '',
    status: 'paused',
    intervalMinutes: 90,
  ...
  items: [
    {
      ...
      nextStep: '设计带导入入口的空状态，并进行一次可用性验证。',
      createdAt,
      updatedAt: createdAt,
    },
    {
      id: 'demo-002',
      channelId: 'demo-system',
      title: '导入失败需要可恢复的错误提示',
      summary: '[示例分析] 网络错误后缺少重试入口，用户无法确定反馈是否已经保存。',
      status: 'investigating',
      kind: 'issue',
      evidence: ['[示例证据] 错误分支只显示通用提示，没有重试或保留输入。'],
      nextStep: '验证重试路径和数据去重，记录可复现步骤。',
      createdAt,
      updatedAt: createdAt,
    },
    {
      id: 'demo-003',
      channelId: 'demo-growth',
      title: '把首条反馈变成激活时刻',
      summary: '[示例假设] 首次导入后提供清晰引导，可能帮助团队更早感受到整理反馈的价值。',
      status: 'open',
      kind: 'hypothesis',
      evidence: ['[示例证据] 试用反馈提到“不知道接下来该做什么”。'],
      nextStep: '先验证用户行为，确认是否存在激活瓶颈。',
      createdAt,
      updatedAt: createdAt,
    },
    {
      ...
      channelId: 'demo-growth',
      title: '为高频反馈生成每周摘要',
      summary: '[示例机会] 将重复出现的反馈归纳为主题，帮助团队讨论下一步优先级。',
      status: 'investigating',
      kind: 'opportunity',
      evidence: ['[示例证据] 产品会议每周手工整理重复反馈。'],
      nextStep: '调查摘要的实际使用场景，形成小范围试验。',
      createdAt,
      updatedAt: createdAt,
    },
    {
      ...
      channelId: 'demo-system',
      title: '反馈列表加载状态已统一',
      summary: '[示例结果] 加载、空状态和错误状态已分开处理。',
      status: 'resolved',
      kind: 'issue',
      evidence: ['[示例证据] 已核对三种展示状态和键盘操作。'],
      nextStep: '持续观察后续使用反馈。',
      createdAt,
      updatedAt: createdAt,
    },
  ...
  events: [
    {
      ...
      runId: '',
      kind: 'system',
      text: '这是明确标注的示例数据，不来自真实运行。示例频道不会执行或自动调度。',
      createdAt,
    },
    {
      id: 'demo-event-2',
      ...
      runId: '',
      kind: 'assistant',
      text: '已整理 **3 个系统完善事项**，分别保留调查证据、状态和下一步。\n\n优先关注首次使用体验，以及导入失败后的恢复路径。',
      createdAt,
    },
  ],
  runtimes: [
...
snapshot.items.forEach((item, index) =>
  Object.assign(item, { projectId: 'demo-atlas', number: index + 1, sourceChannelIds: [item.channelId], revision: 1 })
);
...
const demoRun: Run = {
  ...
  reportError: 'CLI 已完成；本轮没有看板报告。',
  exitCode: 0,
};
snapshot.runs.push(demoRun);
const demoPrompt = '[示例输入] 检查反馈看板的空状态，并记录可验证的改进建议。';
const demoReply = '[示例输出] 已完成检查。空状态需要明确的导入入口和可恢复的失败提示。';
const demoChunks: RunOutputChunk[] = [
  { id: 'demo-chunk-1', runId: demoRun.id, stream: 'prompt', text: demoPrompt, createdAt, sequence: 1 },
  {
    id: 'demo-chunk-2',
    runId: demoRun.id,
    stream: 'stdout',
    text: '[示例原始输出] 正在检查反馈看板…\n',
    createdAt,
    sequence: 2,
  },
  { id: 'demo-chunk-3', runId: demoRun.id, stream: 'final', text: demoReply, createdAt, sequence: 3 },
];
const connection: ConnectionInfo = {
  config: { mode: 'local', host: '', port: 43821, directory: '' },
  connected: true,
  name: '界面预览 · 示例',
};
...
function audit(
  ...
  text: string,
  action: string,
  changes?: WorkspaceEvent['changes']
) {
...
export function previewAPI(): DesktopAPI {
  const api: DesktopAPI = {
    ...
    updateChannel: async (id, data) => {
      ...
      audit(channel.projectId, id, '', '[预览] 更新了频道设置', 'channel.updated', { before, after: channel });
    ...
    getNativeStatus: async () => ({
      available: false,
      connected: false,
      detail: '界面预览；原生对话需要在 Morrow 桌面应用中连接。',
    ...
    getNativeConversation: async (channelId) => ({
      channelId,
      status: await api.getNativeStatus(),
      items: [],
      requests: [],
      hasMore: false,
    }),
    ...
    createItem: async ({ projectId, ...data }) => {
      ...
      const item = {
        ...
        number:
          Math.max(
            0,
            ...snapshot.items.filter((value) => value.projectId === projectId).map((value) => value.number || 0)
          ) + 1,
        sourceChannelIds: data.channelId ? [data.channelId] : [],
        revision: 1,
    ...
    getRun: async (id) => {
      const run = snapshot.runs.find((value) => value.id === id);
      if (!run) throw new Error('运行不存在');
      return { run: structuredClone(run), prompt: demoPrompt, finalOutput: demoReply };
    },
    getRunOutput: async (id, query) => {
      const start = query.after ? demoChunks.findIndex((chunk) => chunk.id === query.after) + 1 : 0;
      const chunks = demoChunks.filter((chunk) => chunk.runId === id).slice(start, start + (query.limit || 60));
      return {
        chunks: structuredClone(chunks),
        hasMore: start + chunks.length < demoChunks.length,
    ...
    getEvents: async (query) => {
      let events = snapshot.events.filter(
        (event) =>
          (!query.projectId || event.projectId === query.projectId) &&
          (!query.channelId || event.channelId === query.channelId) &&
          (!query.itemId || event.itemId === query.itemId) &&
          (!query.runId || event.runId === query.runId)
      );
      if (query.before) {
        const index = events.findIndex((event) => event.id === query.before);
        events = index < 0 ? [] : events.slice(0, index);
      }
      if (query.after) {
        const index = events.findIndex((event) => event.id === query.after);
        events = index < 0 ? [] : events.slice(index + 1);
      }
      const limit = query.limit || 80,
        page = events.slice(-limit);
      return { events: structuredClone(page), hasMore: events.length > limit, cursor: page[0]?.id };
    },
    chooseFolder: unavailable,
    openProjectFolder: unavailable,
