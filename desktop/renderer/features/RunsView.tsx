...
function RunInspector({run,...props}:FeatureProps & {run:Run}){
 ...
 return <div className="run-expanded">
  <div className="run-session-meta"><div><span>触发方式</span><strong>{value.source==='native-app'?'Codex App 对话':value.source==='native-cli'?'Codex CLI 对话':['morrow-chat','nohuman-chat'].includes(value.source||'')?'Morrow 对话':value.trigger==='schedule'?'持续调度':value.trigger==='manual'?'手动运行':'历史记录'}</strong></div><div><span>工作模式</span><strong>{value.permission==='native'?'原生设置':value.permission==='workspace-write'?'工作区编辑':value.permission==='read-only'?'只读跟踪':'—'}</strong></div><div><span>看板报告</span><strong className={['invalid','conflict'].includes(value.reportStatus||'')?'run-report-warning':''}>{value.reportStatus?reportLabels[value.reportStatus]:'旧版记录'}</strong></div></div>
