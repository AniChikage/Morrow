    ...
    <main className="feature-main">
      <div className="feature-toolbar">
        ...
        <label className="feature-select-label">
          项目
          <select aria-label="按项目筛选运行" value={projectId} onChange={(event) => setProjectId(event.target.value)}>
        ...
        <label className="feature-select-label">
          状态
          <select aria-label="按运行状态筛选" value={status} onChange={(event) => setStatus(event.target.value)}>
  ...
  return (
    <div className="run-history">
      ...
      {merged.map((run) => {
        ...
        lastDay = day;
        return (
          <section className="run-history-entry" key={run.id}>
            {heading && (
              <h3 className="run-day">
                <Clock3 size={13} />
                {day}
              </h3>
            )}
            <button
              className={`run-heading ${expanded === run.id ? 'expanded' : ''}`}
              aria-expanded={expanded === run.id}
              onClick={() => setExpanded((previous) => (previous === run.id ? null : run.id))}
            >
              <Terminal size={15} />
              <span className="run-heading-main">
                <strong>
                  {runtimeLabel(run.runtime)}
                  <span>{shortId(run.id)}</span>
                </strong>
                {showChannel && (
                  <small>
                    {project?.name} / {channel?.name || '频道不可用'}
                  </small>
                )}
              </span>
              <time>{runTime(run, run.startedAt)}</time>
              <StatusLabel status={run.status} />
              <ChevronDown className="run-chevron" size={14} />
            </button>
            {expanded === run.id && <RunInspector key={run.id} {...props} run={run} />}
          </section>
        );
      })}
      {(loading || hasMore) && (
...
function RunInspector({ run, ...props }: FeatureProps & { run: Run }) {
  ...
  const [details, setDetails] = useState<RunDetails>(),
    [events, setEvents] = useState<WorkspaceEvent[]>([]),
    [moreEvents, setMoreEvents] = useState(false),
    [eventCursor, setEventCursor] = useState<string>(),
    [loadingEvents, setLoadingEvents] = useState(false);
  const [chunks, setChunks] = useState<RunOutputChunk[]>([]),
    [outputCursor, setOutputCursor] = useState<string>(),
    [moreOutput, setMoreOutput] = useState(true),
    [loadingOutput, setLoadingOutput] = useState(false),
    [loadedOutput, setLoadedOutput] = useState(false);
  const [detailError, setDetailError] = useState(''),
    [eventError, setEventError] = useState(''),
    [outputError, setOutputError] = useState(''),
    [copied, setCopied] = useState(false);
  const mounted = useRef(true),
    request = useRef(0),
    outputBusy = useRef(false);
  ...
  useEffect(() => {
    ...
    void api
      ...
      .catch((failure) => {
        if (mounted.current && current === request.current)
          setDetailError(failure instanceof Error ? failure.message : '无法读取运行详情。');
      });
  ...
  async function loadEvents(before?: string) {
    ...
    try {
      ...
      if (mounted.current) setEventError(failure instanceof Error ? failure.message : '无法读取事件。');
    } finally {
  ...
  async function loadOutput() {
    ...
    try {
      ...
      if (mounted.current) setOutputError(failure instanceof Error ? failure.message : '无法读取原始输出。');
    } finally {
      outputBusy.current = false;
      if (mounted.current) setLoadingOutput(false);
    }
  }
  useEffect(() => {
    if (tab === 'output' && !loadedOutput) void loadOutput();
  }, [tab]);
  const timeline = [
    ...new Map(
      [...events, ...snapshot.events.filter((event) => event.runId === run.id)].map((event) => [event.id, event])
    ).values(),
  ...
  return (
    <div className="run-expanded">
      <div className="run-session-meta">
        <div>
          <span>触发方式</span>
          <strong>
            {value.source === 'native-app'
              ? 'Codex App 对话'
              : ['morrow-chat', 'nohuman-chat'].includes(value.source || '')
                ? 'Morrow 对话'
        ...
        <div>
          <span>看板报告</span>
          <strong className={['invalid', 'conflict'].includes(value.reportStatus || '') ? 'run-report-warning' : ''}>
            {value.reportStatus ? reportLabels[value.reportStatus] : '旧版记录'}
          </strong>
        </div>
      </div>
      {value.summary && (
      ...
      <div className="run-duration">
        ...
        {value.exitCode !== undefined && <span>退出码 {value.exitCode}</span>}
        {channel && (
          <Button variant="ghost" onClick={() => onNavigate({ kind: 'channel', id: channel.id })}>
            打开频道
      ...
      <div className="run-detail-content">
        {tab === 'activity' && (
          <>
            ...
            {moreEvents && (
              <Button variant="ghost" disabled={loadingEvents} onClick={() => void loadEvents(eventCursor)}>
                加载更早动态
              </Button>
            )}
            {timeline.map((event) => (
              <EventLog key={event.id} event={event} runtime={run.runtime} />
            ))}
            {!timeline.length && (
              <p className="subtle">{loadingEvents ? '正在读取动态…' : '此次运行没有可显示的事件。'}</p>
            )}
          </>
        )}
        {tab === 'input' && (
          <>
            {detailError ? (
              ...
              <>
                <p className="run-storage-note">
                  {nativeRun(value)
                    ? '原生 App 对话中记录的本轮输入。'
                    : '发送给原生 CLI 的本轮完整输入。历史会话由 CLI 自身管理。'}
                </p>
                <pre className="run-raw">
                  <code>{details.prompt || '这条历史记录未保存输入副本。'}</code>
                </pre>
              </>
            ) : (
              <p className="subtle">正在读取输入…</p>
            )}
          </>
        )}
        {tab === 'output' && (
          <>
            <p className="run-storage-note">
              {nativeRun(value)
                ? 'SQLite 中保存的原生 App 输出与活动记录。'
                : 'SQLite 中保存的 CLI 原始输出，按接收顺序展示。'}
            </p>
            {outputError && (
              <p className="feature-inline-error" role="alert">
                {outputError}
            ...
            {visibleChunks.map((chunk) => (
              <section className="run-output-chunk" key={chunk.id}>
                <header>
                  <span>{chunk.stream === 'final' ? '最终输出' : chunk.stream}</span>
            ...
            {loadedOutput && !visibleChunks.length && !moreOutput && (
              <p className="subtle">没有原始输出副本；旧版记录可以在执行动态中查看。</p>
            )}
            {(moreOutput || run.status === 'running' || !!outputError) && (
              <Button disabled={loadingOutput} onClick={() => void loadOutput()}>
                {loadingOutput ? '正在读取…' : loadedOutput && moreOutput ? '加载后续输出' : '刷新输出'}
              </Button>
            )}
          </>
        )}
        {tab === 'report' && (
          <>
            {detailError ? (
              ...
              <p className="subtle">正在读取报告…</p>
            ) : details.report ? (
              <pre className="run-raw">
                <code>{JSON.stringify(details.report, null, 2)}</code>
              </pre>
            ) : (
              <p className="subtle">本轮没有可同步的看板报告。执行结果和原生输出仍被保留。</p>
            )}
            {details?.finalOutput && (
              <section className="run-final-answer">
                <h4>原生最终回复</h4>
                <Markdown>{details.finalOutput}</Markdown>
              </section>
            )}
          </>
        )}
