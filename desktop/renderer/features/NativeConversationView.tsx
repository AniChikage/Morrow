...
export function NativeConversationView({ channelId, api, active = true, onState, historyContent, onSent, autonomous=false, compact=false, direction }: Props) {
  ...
  return <section className={`native-conversation ${compact?'native-conversation-compact':''} ${active ? '' : 'native-conversation-hidden'}`} aria-label="Codex 原生对话">
    {(!compact || !ready) && <div className="native-conversation-bar"><span className={`native-sync-label ${ready ? 'is-synced' : ''}`} role="status">{loading ? <LoaderCircle className="spin" size={12} /> : ready ? <Check size={12} /> : <span className="native-sync-dot" />}{syncLabel}</span><span className="native-thread-name" title={conversation?.threadId}>{conversation?.thread?.title || (bound ? 'Codex 对话' : 'Codex App')}</span><div className="feature-toolbar-spacer" /><Button variant="ghost" aria-label="重新连接并同步对话" disabled={loading || loadingOlder} onClick={() => void refresh()}><RefreshCw size={13} /></Button>{!compact && <><Button variant="ghost" disabled={busy || running || !caps?.list} onClick={() => void loadThreads()}><Link2 size={13} />{bound ? '切换对话' : '关联对话'}</Button><Button variant="ghost" disabled={busy} onClick={() => void runAction('open', () => api.openNativeApp(channelId))}>{bound ? '在 App 中打开' : '在 App 中新建'}<ArrowUpRight size={13} /></Button></>}</div>}
    {!conversation?.status.backgroundReady && api.setupNativeBackground && <div className="native-background-setup" role="status"><span>{conversation?.status.backgroundConfigured ? '后台连接已设置，请在当前任务结束后重新打开一次 Codex App。' : '启用后台连接后，可直接在这里新建和恢复对话。'}</span>{!conversation?.status.backgroundConfigured && <Button variant="ghost" disabled={busy} onClick={() => void runAction('setup-background', () => api.setupNativeBackground!())}>启用后台连接</Button>}{conversation?.status.backgroundConfigured && api.restoreNativeBackground && <Button variant="ghost" disabled={busy} onClick={() => void runAction('restore-background', () => api.restoreNativeBackground!())}>撤销设置</Button>}</div>}
...
function NativeRequestView({ request, disabled, canRespond, onRespond, onOpen }: { request: NativeRequest; disabled: boolean; canRespond: boolean; onRespond: (response: unknown) => Promise<unknown>; onOpen: () => void }) {
  ...
  return <div className="native-request"><div><CircleAlert size={15} /><strong>{request.title || (questions.length ? 'Codex 需要你的回答' : 'Codex 需要你的确认')}</strong><Button disabled={sending} onClick={onOpen}>在 App 中处理<ArrowUpRight size={13} /></Button></div>{typeof params.command === 'string' && <pre className="native-request-command">{params.command}</pre>}{typeof params.reason === 'string' && <p className="native-request-reason">{params.reason}</p>}{canRespond && approval && <div className="native-request-actions"><Button disabled={unavailable} onClick={() => void respond({ decision: 'accept' })}>批准本次</Button><Button disabled={unavailable} onClick={() => void respond({ decision: 'decline' })}>拒绝</Button></div>}{canRespond && questions.length > 0 && <form onSubmit={event => { event.preventDefault(); void respond({ answers: Object.fromEntries(questions.map(question => [String(question.id), { answers: [valueFor(String(question.id))] }])) }); }}>{questions.map(question => {
  ...
  })}<Button type="submit" disabled={unavailable || questions.some(question => !valueFor(String(question.id)).trim())}>提交回答</Button></form>}{submitted && <p className="native-request-reason" role="status">已提交，等待 App 更新请求状态…</p>}{error && <div className="feature-inline-error" role="alert">{error}</div>}<NativeDetails className="native-raw-record" summary="请求详情">{() => <pre>{pretty(request.raw)}</pre>}</NativeDetails></div>;
}
