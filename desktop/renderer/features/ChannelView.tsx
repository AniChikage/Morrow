...
export function ChannelView(props: FeatureProps & { id: string }) {
  ...
  </main>{showInspector && (!nativeCodex || details) && <PropertyPanel><section className="property-section"><h3>属性 <button aria-label="编辑频道设置" className="property-icon-button" onClick={() => onEditChannel(channel)}><Settings2 size={14} /></button></h3><Property label="状态"><StatusLabel status={channel.status} /></Property><Property label="引擎">{runtimeLabel(channel.runtime)}</Property><Property label="模型">{nativeCodex ? nativeConversation?.thread?.model || '原生对话设置' : channel.model || 'CLI 默认模型'}</Property><Property label="权限">{nativeCodex ? '原生对话设置' : channel.permission === 'read-only' ? '只读工作空间' : '允许工作区写入'}</Property><Property label="运行间隔">{channel.intervalMinutes} 分钟</Property><Property label="每日上限">{channel.maxRunsPerDay} 次</Property><Button variant="ghost" onClick={() => onEditChannel(channel)}><Settings2 size={14} />编辑设置</Button>{nativeCodex ? <Button variant="ghost" disabled={busy} onClick={() => void onMutate(() => api.openNativeApp(channel.id))}><ArrowUpRight size={14} />在 Codex App 中继续</Button> : <Button variant="ghost" title={nativeBlock || '继续此频道的原生会话；没有会话时打开项目目录中的 CLI。'} disabled={busy || !!nativeBlock} onClick={() => void onMutate(() => api.openNativeSession(channel.id))}><Terminal size={14} />在原生 CLI 中继续</Button>}</section><section className="property-section"><h3>持续目标</h3><div className="property-description"><Markdown>{channel.goal}</Markdown></div></section><section className="property-section"><h3>调度</h3><Property label="上次运行">{formatDate(channel.lastRunAt)}</Property><Property label="下次运行">{channel.status === 'paused' ? '已暂停' : channel.nextRunAt ? formatDate(channel.nextRunAt) : '等待调度'}</Property>{(nativeCodex ? nativeConversation?.threadId : channel.sessionId) && <><h4 className="property-small-label">原生会话</h4><code className="property-session">{nativeCodex ? nativeConversation?.threadId : channel.sessionId}</code></>}</section></PropertyPanel>}</div>;
}
