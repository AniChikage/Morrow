...
export function ChannelView(props: FeatureProps & { id: string }) {
  ...
  useEffect(() => {
    ...
    setHistoryError('');
    setMessage('');
    setTab(nativeCodex ? 'conversation' : 'activity');
    setNativeConversation(null);
    setDetails(false);
  ...
  async function send() {
    ...
    if (await onMutate(() => api.sendMessage(id, value))) {
      setMessage('');
      setTab('activity');
    }
  }
  ...
  const paused =
    channel.autonomyEnabled === undefined
      ? channel.status === 'paused' || channel.status === 'blocked'
      : !channel.autonomyEnabled;
  const demo = !!project?.isDemo;
  const nativeBlock = nativeContinuationBlock(snapshot, channel.projectId);
  const nativeBusy =
    !!nativeConversation?.thread?.activeTurnId ||
    ['active', 'inProgress', 'running'].includes(nativeConversation?.thread?.status || '');
  const nativeRunUnavailable =
    nativeCodex &&
    (!nativeConversationReady(nativeConversation) || !nativeConversation?.status.capabilities.send || nativeBusy);
  const activity = (
    <>
      {historyError && (
      ...
      {events.length > 0 ? (
        <div className="event-timeline">
          ...
          {events.map((event) => (
            <EventLog key={event.id} event={event} runtime={channel.runtime} />
          ))}
        </div>
      ) : (
        !loadingOlder &&
      ...
      <main className="feature-main">
        <header className="channel-heading">
          <div className="channel-title">
            ...
            <h1>{channel.name}</h1>
            {nativeCodex ? (
              <span className="channel-work-status">
                {nativeBusy
                  ? paused
                    ? '正在回应你的指导'
                    : 'Codex 正在工作'
            ...
            {nativeCodex ? (
              <>
                ...
                >
                  ...
                  {paused ? (channel.lastRunAt ? '继续工作' : '开始工作') : '暂停'}
                </Button>
              </>
            ) : (
              <>
        ...
        {(!nativeCodex || details) && (
          <div className="feature-toolbar channel-tabbar">
            ...
            <span className="channel-engine">{runtimeLabel(channel.runtime)}</span>
          </div>
        )}
        {nativeCodex && (
          <NativeConversationView
            ...
            active={tab !== 'runs'}
            onState={setNativeConversation}
            historyContent={tab === 'activity' ? activity : undefined}
            onSent={() => setTab('conversation')}
          />
        )}
        {(tab === 'runs' || (!nativeCodex && tab === 'activity')) && (
          <div className="feature-scroll channel-body">
            {tab === 'activity' ? (
              activity
            ) : (
              <RunHistory {...props} runs={runs} query={{ channelId: id }} showChannel={false} />
            )}
          </div>
        )}
          ...
          <div className="message-composer">
            <div className="composer-box">
              ...
              <div className="composer-footer">
                <span>将在下一次运行时读取</span>
                <Button
                  variant="primary"
                  aria-label="发送消息，快捷键 Command Enter"
            ...
            <span className="composer-hint">⌘ Enter 发送 · 消息不会自动启动运行</span>
          </div>
        )}
      </main>
      {showInspector && (!nativeCodex || details) && (
        <PropertyPanel>
          <section className="property-section">
            <h3>
              属性{' '}
              <button aria-label="编辑频道设置" className="property-icon-button" onClick={() => onEditChannel(channel)}>
                <Settings2 size={14} />
              </button>
            </h3>
            <Property label="状态">
            ...
            {nativeCodex ? (
              <Button
                variant="ghost"
                disabled={busy}
                onClick={() => void onMutate(() => api.openNativeApp(channel.id))}
              >
                <ArrowUpRight size={14} />在 Codex App 中继续
              </Button>
            ) : (
              <Button
                variant="ghost"
          ...
          <section className="property-section">
            ...
            <div className="property-description">
              <Markdown>{channel.goal}</Markdown>
            </div>
          </section>
          <section className="property-section">
            <h3>调度</h3>
            <Property label="上次运行">{formatDate(channel.lastRunAt)}</Property>
            <Property label="下次运行">
              {channel.status === 'paused' ? '已暂停' : channel.nextRunAt ? formatDate(channel.nextRunAt) : '等待调度'}
            </Property>
            {(nativeCodex ? nativeConversation?.threadId : channel.sessionId) && (
              <>
                <h4 className="property-small-label">原生会话</h4>
                <code className="property-session">
                  {nativeCodex ? nativeConversation?.threadId : channel.sessionId}
                </code>
              </>
            )}
