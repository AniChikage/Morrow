...
const statusOrder = boardStatuses;
...
function readPreferences(id: string): ProjectPreferences {
  try { const stored = JSON.parse(localStorage.getItem(`morrow.project-view.${id}`) || localStorage.getItem(`nohuman.project-view.${id}`) || '{}'); return { layout: stored.boardVersion === 2 && stored.layout === 'list' ? 'list' : 'board', status: typeof stored.status === 'string' ? stored.status : 'all', channel: typeof stored.channel === 'string' ? stored.channel : 'all' }; } catch { return defaults; }
}
export function ProjectView(props: FeatureProps & { id: string }) {
  ...
  const changePreferences = (patch: Partial<ProjectPreferences>) => setPreferences(previous => {
    const next = { ...previous, ...patch }; try { localStorage.setItem(`morrow.project-view.${id}`, JSON.stringify({ ...next, boardVersion: 2 })); } catch { /* Keep this view preference for the current session. */ } return next;
  });
