...
export function nativeContinuationBlock(snapshot: Snapshot, projectId: string): string {
  ...
  if (
    channels.some((channel) => !['paused', 'blocked', 'idle'].includes(channel.status) || !!channel.nextRunAt) ||
    snapshot.runs.some((run) => run.status === 'running' && channels.some((channel) => channel.id === run.channelId))
  )
    return '先暂停此项目的全部频道，并等待当前任务结束。';
  return '';
}
