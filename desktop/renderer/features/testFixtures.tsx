...
export function item(patch: Partial<WorkItem> = {}): WorkItem {
  return {
    ...
    title: 'CSV 重试会重复提交',
    summary: '导入超时后再次提交，产生 **两条重复记录**。\n\n### 复现观察\n\n- 首次请求返回超时\n- 重试后重复写入',
    status: 'open',
    kind: 'issue',
    evidence: ['日志包含唯一证据关键词：request-17。', '[官方错误码](https://example.com/import/errors)'],
    nextStep: '验证幂等键，并补充失败后的恢复测试。',
    createdAt: timestamp,
    updatedAt: timestamp,
    ...patch,
  };
...
export function snapshot(): Snapshot {
  return {
    projects: [
      ...
      { id: 'project-other', name: 'Other', path: '', goal: '独立项目', createdAt: timestamp, isDemo: false },
    ],
...
export function featureProps(patch: Partial<FeatureProps> = {}) {
  ...
  const props: FeatureProps = {
    ...
    onNewFeature: vi.fn(),
    onEditFeature: vi.fn(),
    onMutate: vi.fn(async (action: () => Promise<unknown>) => {
      try {
        await action();
        return true;
      } catch {
