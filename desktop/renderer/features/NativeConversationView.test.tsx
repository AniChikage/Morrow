...
describe('native App conversation', () => {
  it('creates and chats through the background without opening the App task', async () => {
    ...
    const unbound = conversation({
      ...
      thread: undefined,
      items: [],
      status: { ...base.status, backgroundReady: true, capabilities: { ...base.status.capabilities, create: true } },
    });
    const { props, api } = setup(unbound);
    api.createNativeThread.mockResolvedValue(conversation({ status: unbound.status }));
    vi.mocked(props.api.getNativeConversation)
      .mockResolvedValueOnce(unbound)
      .mockResolvedValue(conversation({ status: unbound.status }));
    ...
    const input = await screen.findByRole('textbox', { name: '发送到 Codex App 原生对话' });
    fireEvent.change(input, { target: { value: '直接在 Morrow 继续' } });
    fireEvent.keyDown(input, { key: 'Enter', metaKey: true });
    await waitFor(() => expect(api.sendNativeMessage).toHaveBeenCalled());
  ...
  it('loads native image content by bound item and actual block index and supports a zoom preview', async () => {
    ...
    withImage.raw = {
      content: [
        { type: 'text', text: '检查这个截图' },
        { type: 'localImage', path: '/private/native-only.png', name: '产品截图' },
      ],
    };
  ...
  it('renders App steering input as a human message with its image and a discrete native steer marker', async () => {
    const steering: NativeItem = {
      ...
      raw: {
        input: [
          { type: 'text', text: '运行中从 App 追加的指令' },
          { type: 'localImage', path: '/native/steering.png', name: '追加图片' },
        ],
      },
    };
    const marker: NativeItem = {
      id: 'steer-marker',
      turnId: 'active-turn',
      type: 'steered',
      text: '',
      raw: { type: 'steered', id: 'steer-marker' },
    };
    const { props, api } = setup(
      conversation({
        items: [steering, marker],
        thread: { ...conversation().thread!, status: 'active', activeTurnId: 'active-turn' },
      })
    );
    api.getNativeImage.mockResolvedValue({ dataUrl: imageData });
    render(<NativeConversationView channelId="channel-system" api={props.api} />, { wrapper: TestProviders });
  ...
  it('rejects untrusted image URLs and permits retry after a scoped native image read fails', async () => {
    ...
    const { props, api } = setup(conversation({ items: [withImage] }));
    api.getNativeImage
      .mockRejectedValueOnce(new Error('原生图片暂不可读取'))
      .mockResolvedValueOnce({ dataUrl: imageData });
    render(<NativeConversationView channelId="channel-system" api={props.api} />, { wrapper: TestProviders });
  ...
  it('sends image-only messages with opaque attachment IDs, strips previews, and clears accepted attachments', async () => {
    const user = userEvent.setup();
    const { props, api } = setup();
    vi.mocked(props.api.chooseNativeImages).mockResolvedValue([
      { id: 'image-upload-1', name: '界面.png', mimeType: 'image/png', previewUrl: imageData },
    ]);
    ...
    expect(api.sendNativeMessage).toHaveBeenCalledWith('channel-system', {
      ...
      attachments: [{ id: 'image-upload-1', name: '界面.png', mimeType: 'image/png' }],
    });
  ...
  it('keeps uncertain image sends intact and reconciles the same attachment IDs and request ID', async () => {
    ...
    const { props, api } = setup();
    vi.mocked(props.api.chooseNativeImages).mockResolvedValue([
      { id: 'uncertain-image', name: '保留.jpg', mimeType: 'image/jpeg' },
    ]);
    api.sendNativeMessage
      .mockResolvedValueOnce({ state: 'unknown', requestId: 'same' })
      .mockResolvedValueOnce({ state: 'accepted', requestId: 'same' });
    render(<NativeConversationView channelId="channel-system" api={props.api} />, { wrapper: TestProviders });
    await screen.findByText('已同步');
    await user.click(screen.getByRole('button', { name: '添加图片' }));
  ...
  it('renders canonical messages and complete tool output and sends exact text directly to the native thread', async () => {
    ...
    const initial = conversation({
      items: [
        message('human', '从 App 发送的输入', 'user'),
        message('agent'),
        {
          id: 'cmd',
          turnId: 'turn-agent',
          ...
          output: toolOutput,
          status: 'completed',
          raw: { command: 'printf hello', aggregatedOutput: toolOutput },
        },
      ],
    });
    const { props, api } = setup(initial);
    api.sendNativeMessage.mockResolvedValue({ requestId: 'received', state: 'accepted', turnId: 'new-turn' });
    render(<NativeConversationView channelId="channel-system" api={props.api} active={false} />, {
  ...
  it('keeps large tool Markdown, output and raw serialization out of the DOM until each details level is opened', async () => {
    ...
    let rawReads = 0;
    const raw = {
      command: 'read-large-file',
      get detailedPayload() {
        rawReads++;
        return '完整原生记录独有内容';
      },
    };
    const tool: NativeItem = {
      id: 'large-tool',
      turnId: 'turn-large',
      type: 'commandExecution',
      text: '**工具说明未渲染**',
      input: 'read-large-file',
      output,
      status: 'completed',
      raw,
    };
    const { props } = setup(conversation({ items: [tool] }));
    const view = render(<NativeConversationView channelId="channel-system" api={props.api} active={false} />, {
      wrapper: TestProviders,
    });
  ...
  it('automatically opens a tool when it fails, respects a later manual collapse, and labels subagent activity', async () => {
    ...
    vi.mocked(props.api.getNativeConversation).mockResolvedValue(
      conversation({ items: [{ ...tool, status: 'failed' }, subagent] })
    );
  ...
  it('polls App changes, replaces streaming text by native ID, and never duplicates messages', async () => {
    const { props } = setup();
    const view = render(<NativeConversationView channelId="channel-system" api={props.api} />, {
      wrapper: TestProviders,
    });
    await screen.findByText('App 中已有的回复');
    vi.mocked(props.api.getNativeConversation).mockResolvedValue(
      conversation({ items: [message('one', '原生实时完整回复'), message('two', 'App 新增输入', 'user')] })
    );
  ...
  it('keeps history and draft on disconnect, blocks sending, then resumes from canonical App history', async () => {
    ...
    vi.mocked(props.api.getNativeConversation).mockResolvedValue(
      conversation({ items: [message('one'), message('external', '断线期间在 App 发出的消息', 'user')] })
    );
    fireEvent.focus(window);
  ...
  it('retains an ambiguous send and reconciles using the same idempotency key without issuing a second message', async () => {
    ...
    api.sendNativeMessage
      ...
      .mockResolvedValueOnce({ state: 'accepted', requestId: 'same', turnId: 'accepted-turn' });
    render(<NativeConversationView channelId="channel-system" api={props.api} />, { wrapper: TestProviders });
    await screen.findByText('已同步');
    const editor = screen.getByRole('textbox', { name: '发送到 Codex App 原生对话' });
  ...
  it('shows real connection state until a thread snapshot has synced and keeps the App new-task entry available', async () => {
    const user = userEvent.setup();
    const { props, api } = setup(
      conversation({ threadId: undefined, thread: undefined, items: [], lastSyncedAt: undefined })
    );
    render(<NativeConversationView channelId="channel-system" api={props.api} />, { wrapper: TestProviders });
  ...
  it('binds a selected project thread and rejects a stale pre-binding response', async () => {
    ...
    await screen.findByText('App 中已有的回复');
    await act(async () =>
      resolveStale(conversation({ threadId: undefined, thread: undefined, items: [], lastSyncedAt: undefined }))
    );
    expect(screen.getByText('App 中已有的回复')).toBeTruthy();
  ...
  it('loads older native items without duplication and retains the loaded range on live updates', async () => {
    ...
    vi.mocked(props.api.getNativeConversation).mockImplementation(async (_id, query) =>
      query?.before
        ? conversation({ items: [message('one'), message('two'), message('three')], hasMore: false })
        : initial
    );
    render(<NativeConversationView channelId="channel-system" api={props.api} />, { wrapper: TestProviders });
    ...
    fireEvent.focus(window);
    await waitFor(() => expect(props.api.getNativeConversation).toHaveBeenCalledTimes(3));
    expect(screen.getAllByRole('article').map((article) => article.textContent?.match(/消息 \w+/)?.[0])).toEqual([
      '消息 one',
      '消息 two',
      '消息 three',
      '消息 four',
  ...
  it('fills a multi-page reconnect gap before merging the existing visible history', async () => {
    ...
    vi.mocked(props.api.getNativeConversation).mockImplementation(async (_id, query) =>
      query?.before
        ? conversation({ items: [message('two'), message('three')], hasMore: true, cursor: 'two' })
        : conversation({ items: [message('four'), message('five')], hasMore: true, cursor: 'four' })
    );
    fireEvent.focus(window);
  ...
  it('ignores late history from a previous channel and targets interrupt at the current exact turn', async () => {
    ...
    vi.mocked(props.api.getNativeConversation).mockResolvedValue(
      conversation({
        ...
        thread: { ...conversation().thread!, status: 'active', activeTurnId: 'actual-turn' },
      })
    );
    view.rerender(<NativeConversationView channelId="new-channel" api={props.api} />);
    await screen.findByText('App 中已有的回复');
    await act(async () =>
      resolveOld(conversation({ channelId: 'old-channel', items: [message('old', '旧频道迟到记录')] }))
    );
  ...
  it('submits native approvals and structured user answers and waits for canonical request removal', async () => {
    const requests: NativeConversation['requests'] = [
      {
        ...
        status: 'pending',
        raw: { method: 'item/commandExecution/requestApproval', params: { command: 'npm test' } },
      },
      {
        id: 'question',
            ...
            questions: [
              {
                ...
                header: '颜色',
                question: '选择颜色',
                options: [{ label: '灰白', description: '保留现有风格' }],
                isOther: true,
              },
            ],
          },
    ...
    expect(api.respondNativeRequest).toHaveBeenCalledWith('channel-system', 'question', {
      answers: { choice: { answers: ['灰白'] } },
    });
  });
  ...
  it('centers a channel on direction and requires App task association before starting', async () => {
    ...
    const unbound = conversation({
      ...
      status: {
        ...
        capabilities: { ...conversation().status.capabilities, create: true },
      },
    });
  ...
  it('groups tool activity behind one disclosure while keeping Codex progress visible', async () => {
    const tool = (id: string) => ({
      ...message(id, 'tool output', 'tool'),
      type: 'commandExecution',
      status: 'completed',
    });
    const { props } = setup(
      conversation({ items: [message('progress', '正在验证用户首次进入的流程。'), tool('one'), tool('two')] })
    );
    render(<NativeConversationView channelId="channel-system" api={props.api} autonomous compact />, {
      wrapper: TestProviders,
    });
    await screen.findByText('正在验证用户首次进入的流程。');
    expect(screen.queryByText('运行命令')).toBeNull();
    await userEvent.setup().click(screen.getByText('工作过程'));
    expect(screen.getAllByText('运行命令')).toHaveLength(2);
  });
  it('keeps confirmed work context and next-step protocol out of the normal conversation prose', async () => {
    ...
    await screen.findByText('已验证登录错误提示。');
    expect(screen.queryByText('full context from project')).toBeNull();
    expect(screen.queryByText('{"state":"continue"}')).toBeNull();
    await userEvent.setup().click(screen.getByText('继续工作 · 已准备项目上下文'));
    expect(screen.getByText('full context from project')).toBeTruthy();
  });
  it('shares the native composer across conversation and activity tabs and never submits bound chat as a legacy note', async () => {
    ...
    const { props, api } = featureProps({ snapshot: state });
    vi.mocked(props.api.getNativeConversation).mockResolvedValue(
      conversation({ channelId: 'channel-system', lastSyncedAt: undefined })
    );
    render(<ChannelView {...props} id="channel-system" />, { wrapper: TestProviders });
    expect(screen.queryByRole('button', { name: '运行一次' })).toBeNull();
    await user.click(screen.getByRole('button', { name: '工作详情' }));
    ...
    fireEvent.focus(window);
    await waitFor(() =>
      expect((screen.getByRole('button', { name: '开始工作' }) as HTMLButtonElement).disabled).toBe(false)
    );
    fireEvent.change(screen.getByRole('textbox', { name: '发送到 Codex App 原生对话' }), {
      target: { value: '  动态页的指令\n保留原文  ' },
    });
  ...
  it('retains an uncertain native activity send across tabs and reconciles its original request without a second legacy submission', async () => {
    ...
    vi.mocked(props.api.getNativeConversation).mockResolvedValue(conversation());
    api.sendNativeMessage
      .mockResolvedValueOnce({ state: 'unknown', requestId: 'pending' })
      .mockResolvedValueOnce({ state: 'accepted', requestId: 'confirmed' });
    render(<ChannelView {...props} id="channel-system" />, { wrapper: TestProviders });
    await user.click(screen.getByRole('button', { name: '工作详情' }));
    await screen.findByText('已同步');
    await user.click(screen.getByRole('tab', { name: '动态' }));
