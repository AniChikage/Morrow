...
it('labels unavailable native timestamps truthfully for completed App turns and identifies their source', async () => {
  ...
  const current = run('native-without-clock', {
...
it('loads durable history beyond the snapshot, merges pages in newest-first order and keeps live snapshot state', async () => {
  ...
  expect(headings().map((button) => button.textContent?.match(/LATEST|MIDDLE|OLDEST/)?.[0])).toEqual([
    'LATEST',
    'MIDDLE',
  ]);
...
it('keeps CLI completion separate from report validation and displays exact input and paginated original output', async () => {
  ...
  const current = run('detail-run', {
    ...
    reportError: '报告缺少必要字段',
    exitCode: 0,
    sessionId: 'native-session-123',
  });
  props.snapshot.runs = [current];
  vi.mocked(props.api.getRuns).mockResolvedValue({ runs: [current], hasMore: false });
  vi.mocked(props.api.getRun).mockResolvedValue({
    run: current,
    prompt: '实际发送的输入\n保留完整上下文与空行\n\n检查路径 /tmp/project',
    finalOutput: '已完成 **本地验证**。',
    report: { invalidField: '缺少 summary' },
  });
  const chunk = (id: string, stream: RunOutputChunk['stream'], sequence: number, text: string): RunOutputChunk => ({
    id,
    runId: current.id,
    stream,
    sequence,
  ...
  vi.mocked(props.api.getRunOutput)
    .mockResolvedValueOnce({
      chunks: [
        second,
        chunk('prompt', 'prompt', 0, '输入不会冒充输出'),
        first,
        chunk('report', 'report', 4, '报告不会冒充输出'),
      ],
      hasMore: true,
      cursor: 'output-two',
    })
  ...
  expect([...document.querySelectorAll('.run-output-chunk code')].map((node) => node.textContent)).toEqual([
    first.text,
    second.text,
    final.text,
  ]);
...
it('ignores a delayed previous project page and keeps the new scope cursor when loading more history', async () => {
  const { props } = featureProps(),
    user = userEvent.setup(),
    previous = deferred<RunsPage>();
  const oldRun = run('oldscp-run'),
    newRun = run('newscp-run', { projectId: 'project-other', channelId: 'channel-other' });
  const olderNewRun = run('newold-run', {
  ...
  vi.mocked(props.api.getRuns).mockImplementation((query) => {
    ...
    return Promise.resolve(
      query.before
        ? { runs: [olderNewRun], hasMore: false }
        : { runs: [newRun], hasMore: true, cursor: 'new-project-cursor' }
    );
  });
  const view = render(<RunHistory {...props} runs={[]} query={{ projectId: 'project-atlas' }} />, {
  ...
  await act(async () => {
    previous.resolve({ runs: [oldRun], hasMore: true, cursor: 'old-project-cursor' });
  ...
  expect(props.api.getRuns).toHaveBeenLastCalledWith({
    projectId: 'project-other',
    before: 'new-project-cursor',
    limit: 80,
  });
});
...
it('does not leak delayed input details after opening a different run', async () => {
  const { props } = featureProps(),
    user = userEvent.setup(),
    firstDetails = deferred<RunDetails>();
  const first = run('firsta-run'),
    second = run('second-run', { startedAt: '2026-09-06T02:00:00.000Z' });
  props.snapshot.runs = [first, second];
  vi.mocked(props.api.getRuns).mockResolvedValue({ runs: [first, second], hasMore: false });
  vi.mocked(props.api.getRun).mockImplementation((id) =>
    id === first.id
      ? firstDetails.promise
      : Promise.resolve({ run: second, prompt: '第二次运行独立输入', finalOutput: '' })
  );
  render(<RunsView {...props} />, { wrapper: TestProviders });
  await user.click(headings()[0]);
  await user.click(screen.getByRole('tab', { name: '本轮输入' }));
  ...
  await act(async () => {
    firstDetails.resolve({ run: first, prompt: '旧运行不应泄漏的输入', finalOutput: '' });
...
it('retains visible live runs when durable history fails and retries the same channel scope', async () => {
  const { props } = featureProps(),
    user = userEvent.setup(),
    latest = run('active-run', { status: 'running' });
  vi.mocked(props.api.getRuns)
    .mockRejectedValueOnce(new Error('历史数据库暂不可用'))
    .mockResolvedValueOnce({ runs: [latest], hasMore: false });
...
it('keeps the durable page range authoritative when a large snapshot includes unloaded runs with identical timestamps', async () => {
  ...
  const live = snapshot.map((value) =>
    value.id === snapshot[179].id ? { ...value, status: 'running' as const, finishedAt: '' } : value
  );
