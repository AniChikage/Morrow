...
describe('provider event compatibility', () => {
  it('shows a structured shell command and output without its transport envelope', () => {
    const result = toolPresentation(
      event({
        text: '{"provider":"raw envelope"}',
        detail: {
          type: 'tool_result',
          tool: 'shell',
          input: { command: 'rg -n retry src' },
          output: 'src/import.ts:44: retry()',
          status: 'completed',
          sequence: 3,
        },
      })
      ...
      event({
        detail: {
          ...
          output: [
            ...
            { type: 'text', text: 'Workspace is read-only' },
          ],
          status: 'failed',
          toolCallId: 'call-1',
        },
  ...
  it('decodes legacy Claude tool calls and tool result messages', () => {
    const call = toolPresentation(
      event({
        text: JSON.stringify({
          type: 'assistant',
          message: { content: [{ type: 'tool_use', name: 'Read', input: { file_path: '/project/README.md' } }] },
        }),
      })
    );
    ...
    const result = toolPresentation(
      event({
        text: JSON.stringify({
          type: 'user',
          message: {
            content: [{ type: 'tool_result', content: [{ type: 'text', text: 'missing file' }], is_error: true }],
          },
        }),
      })
    );
  ...
  it('keeps malformed or unknown logs inspectable instead of discarding them', () => {
    ...
    expect(readableEventText(event({ kind: 'assistant', text: 'A plain text finding.' }))).toBe(
      'A plain text finding.'
    );
    expect(readableEventText(event({ kind: 'assistant', text: '{"unrecognized":"retain me"}' }))).toBe(
      '{"unrecognized":"retain me"}'
    );
  });
  it('extracts a readable agent answer from legacy envelopes while preserving Markdown', () => {
    ...
    expect(
      readableEventText(
        event({
          kind: 'assistant',
          text: JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: answer } }),
        })
      )
    ).toBe(answer);
    expect(readableEventText(event({ kind: 'result', text: JSON.stringify({ summary: answer, items: [] }) }))).toBe(
      answer
    );
  });
});
