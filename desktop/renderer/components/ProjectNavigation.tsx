...
export function ProjectNavigation({
  ...
  useEffect(() => {
    if (activeProjectId)
      setCollapsed((previous) => (previous[activeProjectId] ? { ...previous, [activeProjectId]: false } : previous));
  ...
  for (const channel of channels) {
    ...
    else byProject.set(channel.projectId, [channel]);
  }
          ...
          <section className="nav-project" aria-label={`${project.name} 项目`} key={project.id}>
            <div className={`nav-project-row ${activeProjectId === project.id ? 'current-project' : ''}`}>
              <button
                ...
                title={`${expanded ? '收起' : '展开'}项目`}
                aria-expanded={expanded}
                aria-controls={groupId}
                onClick={() => setCollapsed((previous) => ({ ...previous, [project.id]: expanded }))}
              >
                <Folder size={15} />
              ...
              <button
                ...
                title="新建频道"
                onClick={() => {
                  setCollapsed((previous) => ({ ...previous, [project.id]: false }));
            ...
            <div id={groupId} className="nav-project-content" hidden={!expanded}>
              <button
                className={`nav-item nav-board ${route?.kind === 'project' && activeProjectId === project.id ? 'selected' : ''}`}
                aria-label={`${project.name} 的看板`}
                aria-current={route?.kind === 'project' && activeProjectId === project.id ? 'page' : undefined}
                onClick={() => onNavigate({ kind: 'project', id: project.id }, true)}
              >
                <Columns3 size={14} />
                <span>看板</span>
              </button>
              <div className="nav-project-channels" role="group" aria-label={`${project.name} 的频道`}>
                ...
                {children.map((channel) => (
                  <button
                    className={`nav-item nav-channel ${route?.kind === 'channel' && channel.id === route.id ? 'selected' : ''}`}
                    key={channel.id}
                    title={channel.name}
                    aria-current={route?.kind === 'channel' && channel.id === route.id ? 'page' : undefined}
                    onClick={() => onNavigate({ kind: 'channel', id: channel.id })}
                  >
                    <Hash size={14} />
                    <span>{channel.name}</span>
                    {channel.status === 'running' && <span className="running-dot" role="img" aria-label="正在运行" />}
                    {channel.status === 'blocked' && (
                      <AlertCircle className="nav-trailing warning" aria-label="需要处理" />
                    )}
                  </button>
                ))}
                {!children.length && (
                  <button className="nav-item nav-channel nav-channel-empty" onClick={() => onNewChannel(project.id)}>
                    <Plus size={14} />
                    <span>新建频道</span>
                  </button>
                )}
