...
function ProjectDialog({onClose,onNavigate}:{onClose:()=>void;onNavigate:(route:Route,newTab?:boolean)=>void}) {
 ...
 return <Modal title="打开项目文件夹" description="把已有项目接入 NoHuman，统一跟踪功能与持续改进。" onClose={onClose}>
...
function SettingsDialog({onClose}:{onClose:()=>void}) {
 const {api,connection,busy,mutate,error,reset,setConnectionInfo,clearError}=useWorkspace();const [config,setConfig]=useState<ConnectionConfig>(connection?.config||{mode:'local',host:'',port:43821,directory:'~/.local/share/nohuman'});const [saved,setSaved]=useState(false);const remoteConfig=useRef<ConnectionConfig>(connection?.config.mode==='ssh'?connection.config:{mode:'ssh',host:'',port:43821,directory:'~/.local/share/nohuman'});
 ...
 return <Modal title="设置" description="管理桌面应用连接的执行位置。" onClose={onClose}><form onSubmit={submit}><fieldset className="settings-fields" disabled={busy}><div className="settings-section-title">执行位置</div><div className="segmented settings-mode"><button type="button" aria-pressed={config.mode==='local'} onClick={()=>changeMode('local')}><Laptop size={14}/>本机 Mac</button><button type="button" aria-pressed={config.mode==='ssh'} onClick={()=>changeMode('ssh')}><Server size={14}/>远程 SSH</button></div>{config.mode==='ssh'?<><Field title="SSH 主机" hint="使用已有 SSH 配置中的主机别名或 user@host。"><input value={config.host} onChange={e=>{setSaved(false);setConfig(c=>({...c,host:e.target.value}));}} placeholder="dev-box" required/></Field><div className="form-row"><Field title="服务端口"><input type="number" min={1} max={65535} value={config.port} onChange={e=>{setSaved(false);setConfig(c=>({...c,port:Number(e.target.value)}));}} required/></Field><Field title="远程数据目录"><input value={config.directory} onChange={e=>{setSaved(false);setConfig(c=>({...c,directory:e.target.value}));}} placeholder="~/.local/share/nohuman" required/></Field></div><p className="form-note">远端需要先启动 NoHuman 执行服务。通过已有 SSH 登录连接，执行和数据保留在远端。</p></>:<p className="form-note">项目和执行记录保存在这台 Mac。关闭界面后，独立执行服务继续运行。</p>}</fieldset>{error&&<p role="alert" className="form-error">{error}</p>}{saved&&<p role="status" className="settings-saved"><Check size={14}/>已连接到{config.mode==='local'?'本机 Mac':config.host}</p>}<div className="form-actions"><Button variant="ghost" onClick={()=>void mutate(()=>api.openDataFolder())}><FolderOpen/>数据目录</Button><span className="spacer"/><Button onClick={onClose}>完成</Button><Button variant="primary" type="submit" disabled={busy||config.mode==='ssh'&&!config.host.trim()}>{busy?'正在连接…':'连接'}</Button></div></form><div className="settings-about"><span>NoHuman</span><span>0.3.0 · Electron / React</span></div></Modal>;
}
