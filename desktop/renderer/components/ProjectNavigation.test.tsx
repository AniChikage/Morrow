...
it('remembers independent collapsed projects and reveals a channel reached through another tab', async () => {
  ...
  expect(screen.queryByRole('group', { name: 'Atlas 示例项目 的频道' })).toBeNull();
  next.rerender(
    <ProjectNavigation {...props} route={{ kind: 'channel', id: 'channel-growth' }} activeProjectId="project-atlas" />
  );
  ...
  next.rerender(
    <ProjectNavigation
      ...
      route={{ kind: 'channel', id: 'channel-growth' }}
