            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Text("使用 \(store.connectionName) 上的 CLI 登录状态。检测仅确认安装；登录与配额在执行时验证。")
                        .font(.system(size: 12)).foregroundStyle(Color.morrowSecondary).lineSpacing(3)
                        .padding(.top, 16)
            ...
            HStack(spacing: 8) {
                HStack(spacing: 8) {
                    Image(systemName: runtime.id == "claude" ? "sun.max" : "terminal")
                        .font(.system(size: 14)).foregroundStyle(Color.morrowSecondary)
                        .frame(width: 17)
                    Text(runtime.name).font(.system(size: 13, weight: .medium))
                }.frame(maxWidth: .infinity, alignment: .leading)
                Text(runtime.version.isEmpty ? "—" : runtime.version)
                    .font(.system(size: 11, design: .monospaced)).foregroundStyle(Color.morrowSecondary)
                    .lineLimit(1).help(runtime.version)
                    .frame(width: 150, alignment: .leading)
                HStack(spacing: 5) {
                    Circle().fill(runtime.available ? Color.morrowGreen : Color.morrowSecondary).frame(width: 5, height: 5)
                    Text(runtime.available ? "已检测到" : "未检测到").font(.system(size: 11))
                }.foregroundStyle(Color.morrowSecondary).frame(width: 88, alignment: .trailing)
            }
            Text(runtime.detail).font(.system(size: 12)).foregroundStyle(Color.morrowSecondary)
                .lineSpacing(3).textSelection(.enabled)
            if !runtime.path.isEmpty {
                Text(runtime.path).font(.system(size: 11, design: .monospaced)).foregroundStyle(Color.morrowSecondary)
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 8) {
                ...
                Text("\(runs.count)").font(.system(size: 11)).foregroundStyle(Color.morrowSecondary)
                Spacer()
                Text("最新优先").font(.system(size: 11)).foregroundStyle(Color.morrowSecondary)
            }
            .padding(.horizontal, 18).frame(height: 44)
            .overlay(alignment: .bottom) { Rectangle().fill(Color.morrowLine).frame(height: 1) }
            ScrollView {
