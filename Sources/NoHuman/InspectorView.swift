...
struct InspectorView: View {
    ...
    private var projectDetails: some View {
        Group {
            VStack(alignment: .leading, spacing: 15) {
                Image(systemName: "folder.fill").symbolRenderingMode(.hierarchical).font(.system(size: 25)).foregroundStyle(Color.nhSecondary)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                Text("所有频道共享这个项目目标。").font(.system(size: 12)).foregroundStyle(Color.nhSecondary)
            }
            VStack(alignment: .leading, spacing: 14) {
                section("资源")
                if let project = store.selectedProject, !project.path.isEmpty {
                    ...
                    Text("示例项目，未关联实际目录。").font(.system(size: 12)).foregroundStyle(Color.nhSecondary)
                }
            }
        }
    }
    ...
    private func channelDetails(_ channel: Channel) -> some View {
        Group {
            VStack(alignment: .leading, spacing: 14) {
                Image(systemName: "number").font(.system(size: 24, weight: .light)).foregroundStyle(Color.nhSecondary)
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    section("属性")
                    Spacer()
                    Button("编辑") { showSettings = true }.buttonStyle(.plain).font(.system(size: 12)).foregroundStyle(Color.nhSecondary)
                }
            ...
            if store.selectedProject?.isDemo == true {
                Text("示例频道，不会执行或自动调度。").font(.system(size: 12)).foregroundStyle(Color.nhSecondary).lineSpacing(3)
            }
        }
    }
    ...
    private func itemDetails(_ item: WorkItem) -> some View {
        Group {
            VStack(alignment: .leading, spacing: 13) {
                HStack {
                    Label(kindTitle(item.kind), systemImage: kindSymbol(item.kind)).font(.system(size: 12)).foregroundStyle(Color.nhSecondary)
                    Spacer()
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(spacing: 7) { section("证据"); Text("\(item.evidence.count)").font(.system(size: 12)).foregroundStyle(Color.nhSecondary) }
                if item.evidence.isEmpty {
                    Text("尚未附带证据，保留为待验证。").font(.system(size: 12)).foregroundStyle(Color.nhSecondary)
                }
                ForEach(Array(item.evidence.enumerated()), id: \.offset) { index, evidence in
                    VStack(alignment: .leading, spacing: 7) {
                        Label("证据 \(index + 1)", systemImage: "link").font(.system(size: 11)).foregroundStyle(Color.nhSecondary)
                        Text(evidence).font(.system(size: 12)).lineSpacing(4).textSelection(.enabled).fixedSize(horizontal: false, vertical: true)
                    }.padding(.leading, 10).overlay(alignment: .leading) { Rectangle().fill(Color.nhLine).frame(width: 2) }
                }
            }
            VStack(alignment: .leading, spacing: 13) {
                section("下一步")
    ...
    private func property<Value: View>(_ name: String, @ViewBuilder value: () -> Value) -> some View {
        HStack(alignment: .center, spacing: 8) {
            Text(name).foregroundStyle(Color.nhSecondary).frame(width: 70, alignment: .leading)
    ...
    private func textProperty(_ name: String, value: String) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(name).font(.system(size: 12)).foregroundStyle(Color.nhSecondary)
