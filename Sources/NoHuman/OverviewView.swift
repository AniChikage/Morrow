...
struct OverviewView: View {
    ...
    private func toolbarLabel(_ title: String, symbol: String, active: Bool = false) -> some View {
        ...
        .foregroundStyle(active ? Color.nhInk : Color.nhSecondary)
        .padding(.horizontal, 8).frame(height: 27)
        .background(active ? Color.black.opacity(0.035) : .white, in: RoundedRectangle(cornerRadius: 5))
        .overlay(RoundedRectangle(cornerRadius: 5).stroke(Color.nhLine, lineWidth: 1))
    }
                ...
                ForEach(visibleStatuses, id: \.self) { status in
                    ...
                    if !collapsedGroups.contains(status) {
                        ForEach(items.filter { $0.status == status }) { item in
                            ...
                            Rectangle().fill(Color.nhLine.opacity(0.6)).frame(height: 1).padding(.leading, 48)
                        }
                    }
                }
            }.padding(.top, 12)
        ...
        Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: collapsedGroups.contains(status) ? "chevron.right" : "chevron.down")
                    .font(.system(size: 9, weight: .medium)).frame(width: 12).foregroundStyle(Color.nhSecondary)
                OverviewStatusSymbol(status: status)
                Text(statusTitle(status)).font(.system(size: 12, weight: .medium))
                Text("\(items.filter { $0.status == status }.count)").font(.system(size: 12)).foregroundStyle(Color.nhSecondary)
                Spacer()
            }
                            ...
                            ForEach(items.filter { $0.status == status }) { item in
                                Button {
                                    ...
                                    VStack(alignment: .leading, spacing: 13) {
                                        Text(kindTitle(item.kind)).font(.system(size: 11)).foregroundStyle(Color.nhSecondary)
                                    ...
                                    .overlay(RoundedRectangle(cornerRadius: 6).stroke(Color.nhLine, lineWidth: 1))
                                }.buttonStyle(.plain)
                            }
                        }.frame(width: 240)
                    }
    ...
    private var channelList: some View {
        VStack(spacing: 0) {
            if visibleChannels.isEmpty {
                VStack(spacing: 14) {
                    Image(systemName: "number").font(.system(size: 28, weight: .light)).foregroundStyle(Color.nhSecondary)
                    Text("还没有持续频道").font(.system(size: 14)).foregroundStyle(Color.nhSecondary)
                    Text("创建一个频道，为项目持续关注一个方向。").font(.system(size: 13)).foregroundStyle(Color.nhSecondary)
    ...
    private var emptyState: some View {
        VStack(spacing: 13) {
            Image(systemName: "checklist").font(.system(size: 32, weight: .regular)).foregroundStyle(Color.nhSecondary.opacity(0.85)).padding(.bottom, 3)
            Text(hasFilters ? "没有符合筛选条件的发现" : "还没有项目发现").font(.system(size: 14)).foregroundStyle(Color.nhSecondary)
            Text(hasFilters ? "调整筛选条件，查看项目中的其他发现。" : "运行一个持续频道，发现和证据会保存在这里。")
                .font(.system(size: 13)).foregroundStyle(Color.nhSecondary).multilineTextAlignment(.center)
            HStack(spacing: 8) {
                if hasFilters {
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 10) {
                ...
                Text(kindTitle(item.kind)).font(.system(size: 11)).foregroundStyle(Color.nhSecondary).fixedSize()
                if let channelName {
                    Text("# \(channelName)").font(.system(size: 12)).foregroundStyle(Color.nhSecondary).lineLimit(1).frame(width: 104, alignment: .leading)
                }
                if let runtimeName {
                    Text(runtimeName).font(.system(size: 12)).foregroundStyle(Color.nhSecondary).lineLimit(1).frame(width: 82, alignment: .leading)
                }
                HStack(spacing: 4) {
                    Image(systemName: "link").font(.system(size: 10))
                    Text("\(item.evidence.count)").font(.system(size: 12))
                }.foregroundStyle(Color.nhSecondary).frame(width: 32, alignment: .trailing)
            }
...
private struct OverviewStatusSymbol: View {
    ...
    private var tint: Color {
        ...
        case "blocked": return .nhOrange
        case "investigating", "running": return .nhAccent
...
private struct OverviewChannelRow: View {
    ...
    var body: some View {
        Button(action: action) {
            HStack {
                HStack(spacing: 10) {
                    Image(systemName: "number").font(.system(size: 13)).foregroundStyle(Color.nhSecondary).frame(width: 16)
                    Text(channel.name).font(.system(size: 13)).lineLimit(1)
                }.frame(maxWidth: .infinity, alignment: .leading)
                Text(runtimeTitle(channel.runtime)).font(.system(size: 12)).foregroundStyle(Color.nhSecondary).frame(width: 98, alignment: .leading)
                Text("\(channel.intervalMinutes) 分钟").font(.system(size: 12)).foregroundStyle(Color.nhSecondary).frame(width: 74, alignment: .leading)
                HStack(spacing: 6) {
                    OverviewStatusSymbol(status: channel.status)
                    Text(statusTitle(channel.status)).font(.system(size: 12)).foregroundStyle(Color.nhSecondary).lineLimit(1)
                }.frame(width: 114, alignment: .leading)
            }
            .padding(.horizontal, 22).frame(height: 44)
            .background(hovered ? Color.black.opacity(0.025) : .clear)
            .contentShape(Rectangle())
            .overlay(alignment: .bottom) { Rectangle().fill(Color.nhLine.opacity(0.6)).frame(height: 1).padding(.leading, 48) }
