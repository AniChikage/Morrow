...
struct ContentView: View {
        ...
        HStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                workspace
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 10).stroke(Color.nhLine, lineWidth: 1))
                    .padding(.trailing, 6).padding(.bottom, 6)
            }
        }
        .ignoresSafeArea(.container, edges: .top)
        .background(Color.nhSidebar)
        .foregroundStyle(Color.nhInk)
                ...
                HStack(spacing: 4) {
                    ForEach(store.snapshot.projects) { project in
                        Button { store.selectProject(project) } label: {
                            HStack(spacing: 7) {
                                Image(systemName: "folder").font(.system(size: 12)).foregroundStyle(Color.nhSecondary)
                            ...
                            .foregroundStyle(project.id == store.selectedProjectID ? Color.nhInk : Color.nhSecondary)
                            .background(project.id == store.selectedProjectID ? Color.nhCanvas : Color.clear, in: RoundedRectangle(cornerRadius: 7))
                            .overlay(RoundedRectangle(cornerRadius: 7).stroke(project.id == store.selectedProjectID ? Color.nhLine : Color.clear, lineWidth: 1))
                        }.buttonStyle(.plain)
                    }
                }
            }
        ...
        HStack(spacing: 8) {
            if store.selection == "runtimes" {
                ...
                Button("项目") { store.selection = "overview"; store.selectedChannelID = nil; selectedItemID = nil }
                    .buttonStyle(.plain).foregroundStyle(Color.nhSecondary)
            }
            if let project = store.selectedProject, store.selection != "runtimes" {
                Image(systemName: "chevron.right").font(.system(size: 9)).foregroundStyle(Color.nhSecondary)
                Text(project.name).fontWeight(.medium).lineLimit(1)
                if store.selection != "overview" {
                    Image(systemName: "chevron.right").font(.system(size: 9)).foregroundStyle(Color.nhSecondary)
                    Text(pageTitle).foregroundStyle(Color.nhSecondary).lineLimit(1)
                }
    ...
    private var welcome: some View {
        VStack(spacing: 13) {
            Image(systemName: "folder.badge.plus").font(.system(size: 30, weight: .regular)).foregroundStyle(Color.nhSecondary).padding(.bottom, 4)
            Text("还没有项目").font(.system(size: 15, weight: .medium))
            Text("接入代码目录，添加持续负责项目的 AI 频道。").font(.system(size: 13)).foregroundStyle(Color.nhSecondary)
            HStack(spacing: 8) {
                NHButton(title: "新建项目", symbol: "plus") { showNewProject = true }
                Button("浏览示例") { Task { await store.loadDemo() } }.buttonStyle(.plain).font(.system(size: 12)).foregroundStyle(Color.nhSecondary)
            }.disabled(store.isBusy || !store.isConnected).padding(.top, 2)
            if !store.isConnected { Text("正在连接执行服务…").font(.system(size: 12)).foregroundStyle(Color.nhSecondary) }
