...
这是 `yukun` 分支（0.14.1）。远端 `main`（0.10.0）自 `6246930` 起改为「直接用 Codex CLI、去掉与桌面 App 的耦合」；本分支是另一条线，执行入口以 Codex App follower 为准。两条线在执行路线上互斥，本 README 其余部分描述的都是本分支的行为。

| 差异 | main（0.10.0） | 本分支 yukun（0.14.1） |
| --- | --- | --- |
