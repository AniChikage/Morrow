...
Morrow 只支持 Codex。新频道默认沿用 Codex App 自己的权限设置（以 App 中当前的设置为准；单人使用时建议在 App 里选择完整访问），需要时可在频道设置中收紧为只读或工作区写入。早期版本留下的 Claude Code / Trae 记录保持可读，但不再执行。
...
1. 打开已有项目文件夹，写下目标。默认频道处于暂停状态，接入不会立即执行；开始工作后沿用 Codex App 的权限设置。
